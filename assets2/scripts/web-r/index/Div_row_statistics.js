async function Div_row_statistics(){
	function Col_lg_4(props) {
		return (
			<div class="col-lg-4">
				<h3 class="fw-400 color-oxford">{props.content}</h3>
	...
	function Result(props) {
		return (
			<div class="container">
				...
				<div class="row justify-content-center">
					<Col_lg_4 title="가입자 수" content={props.result} />
					<Col_hidden />
					<Col_lg_4 title="일 평균 방문자 수" content="ㅇㅇ명" />
					<Col_hidden />
					<Col_lg_4 title="총 페이지 뷰" content="ㅇㅇ건" />
					<Col_finish />
				</div>
			</div>
		)
	...
	const data = await fetch("/seminar/ajax_get_table_seminar_list")
