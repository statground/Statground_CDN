	...
	$.ajax({
		...
		success:function(data){
			...
			if (data.access_mail_letter == 1) {
				html = '<button class="btn btn-warning" tabindex="5" onclick="location.href=\'/myInfo/newsLetterSubmit/\'">뉴스레터 구독 해지</button>'
				$("#btnList").append(html)
			} else {
				html = '<button class="btn btn-warning" tabindex="5" onclick="location.href=\'/myInfo/newsLetterSubmit/\'">뉴스레터 구독</button>'
				$("#btnList").append(html)
			}
