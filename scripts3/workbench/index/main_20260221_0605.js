...
html.dark body{
  ...
  color-scheme: dark;
  /* match book-workbench vibe: subtle radial gradients */
  background-image:
    radial-gradient(1200px 600px at 20% 10%, rgba(59,130,246,0.14), rgba(2,6,23,0) 60%),
    radial-gradient(1000px 500px at 90% 0%, rgba(99,102,241,0.12), rgba(2,6,23,0) 55%);
...
html.dark .dark\:bg-slate-900\/70, html.dark .dark\:bg-slate-900\/80{
  background-color: rgba(2,6,23,0.35) !important; /* keep consistent */
}

/* Hero large panel: give gentle vertical gradient so it doesn't look "banded" */
html.dark div[class*="bg-white/80"][class*="rounded-3xl"]{
  background-image: linear-gradient(180deg, rgba(15,23,42,0.55), rgba(2,6,23,0.45)) !important;
}
...
/* Icon chip background + icon visibility (book icon) */
html.dark .bg-white\/70, html.dark .bg-white\/80{
  color:#e5e7eb !important; /* ensures SVG currentColor is light */
}
html.dark .bg-white\/70 svg, html.dark .bg-white\/80 svg{
  color:#e5e7eb !important;
}
