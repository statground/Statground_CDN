...
(function () {
  function set_main() {
    const rootEl = document.getElementById("div_main");
        // --- Tailwind Play CDN safelist (external JS classes are not scanned)
    // This page builds most UI via external main.js; Tailwind CDN won't see those class tokens unless
    // they appear in the DOM. We inject a hidden node containing all class tokens used in this file.
    (function ensureTailwindSafelist() {
      try {
        if (document.getElementById('sg_tw_safelist')) return;
        const el = document.createElement('div');
        el.id = 'sg_tw_safelist';
        ...
        el.style.opacity = '0';
        el.className = "bg-rose-50 bg-slate-50 bg-slate-900 bg-white border border-b border-rose-200 border-slate-200 dark:bg-rose-950/30 dark:bg-slate-900 dark:bg-slate-950 dark:bg-white dark:border-rose-900 dark:border-slate-700 dark:border-slate-800 dark:focus:ring-slate-600 dark:hover:bg-slate-950 dark:hover:text-white dark:text-rose-200 dark:text-slate-200 dark:text-slate-300 dark:text-slate-400 dark:text-slate-700 dark:text-slate-800 dark:text-slate-900 dark:text-white flex flex-1 flex-wrap focus:outline-none focus:ring-2 focus:ring-slate-400 font-black font-bold font-extrabold gap-4 gap-x-4 gap-y-1 h-10 h-20 h-6 hover:bg-slate-50 hover:opacity-90 hover:text-slate-900 items-center justify-between line-clamp-2 max-w-6xl mb-8 md:text-4xl md:text-base min-w-0 mt-1 mt-2 mt-3 mt-4 mx-auto object-cover p-4 p-6 px-4 px-5 px-6 py-2 py-3 py-5 rounded-2xl rounded-lg rounded-xl shadow-sm shrink-0 text-3xl text-lg text-rose-700 text-slate-200 text-slate-300 text-slate-500 text-slate-600 text-slate-700 text-slate-900 text-sm text-white text-xs tracking-tight transition truncate w-14 w-6 w-full";
