...
(function () {
  ...
  // ------------------------------------------------------------
  // Dark-mode fallback (main.js-only)
  // ------------------------------------------------------------
  // Tailwind CDN sometimes does not generate/apply `dark:*` utilities for classes that exist only
  // inside external JS-rendered React trees. To keep the Book Workbench readable, we add a scoped
  // CSS fallback that activates when <html class="dark"> is present.
  (function ensureDarkFallbackStyles(){
    if (document.getElementById('sgwb-dark-fallback-style')) return;
    const style = document.createElement('style');
    style.id = 'sgwb-dark-fallback-style';
    style.textContent = `
      /* scope: Book Workbench only */
      .sgwb-root { background: transparent; }
      html.dark .sgwb-root { background: #0b1220; }
      html.dark .sgwb-root .sgwb-surface {
        background: #0f172a !important;
        border-color: #334155 !important;
      }
      html.dark .sgwb-root .sgwb-card {
        background: #0f172a !important;
        border-color: #334155 !important;
      }
      html.dark .sgwb-root, 
      html.dark .sgwb-root p,
      html.dark .sgwb-root span,
      html.dark .sgwb-root div,
      html.dark .sgwb-root h1,
      html.dark .sgwb-root h2,
      html.dark .sgwb-root h3,
      html.dark .sgwb-root h4,
      html.dark .sgwb-root li,
      html.dark .sgwb-root label {
        color: #e2e8f0;
      }
      html.dark .sgwb-root .sgwb-muted { color: #94a3b8 !important; }
      html.dark .sgwb-root a { color: #cbd5e1; }
      html.dark .sgwb-root a:hover { color: #ffffff; }
      html.dark .sgwb-root input,
      html.dark .sgwb-root textarea,
      html.dark .sgwb-root select {
        background: #0b1220 !important;
        border-color: #334155 !important;
        color: #e2e8f0 !important;
      }
      html.dark .sgwb-root input::placeholder { color: #64748b !important; }
      html.dark .sgwb-root button.sgwb-primary {
        background: #e2e8f0 !important;
        color: #0b1220 !important;
      }
      html.dark .sgwb-root button.sgwb-ghost:hover { background: rgba(148,163,184,.12) !important; }
    
      /* header/footer fallback (common) */
      html.dark body { background: #0b1220 !important; color: #e5e7eb !important; }
      html.dark nav { background: #0b1220 !important; border-bottom: 1px solid #1f2937 !important; }
      html.dark nav a, html.dark nav span, html.dark nav div { color: #e5e7eb !important; }
      html.dark #theme-toggle, html.dark #lang-toggle, html.dark #mobile-theme-toggle, html.dark #mobile-lang-toggle {
        color: #e5e7eb !important;
      }
      html.dark footer { background: #0b1220 !important; border-top: 1px solid #1f2937 !important; color: #cbd5e1 !important; }
...
const fetchRecent = async () => {
      ...
      return (
        React.createElement("div", { className: "w-full sgwb-root" },
          React.createElement("div", { className: "max-w-6xl mx-auto" },

            React.createElement("div", { className: "mb-8" },
              React.createElement("a", { href: backHref, className: "text-sm font-bold text-slate-500 dark:text-slate-400 sgwb-muted hover:text-slate-900 dark:hover:text-white" },
              ...
              React.createElement("p", { className: "mt-3 text-sm md:text-base text-slate-600 dark:text-slate-300 sgwb-muted" },
                t(lang, "wb.book.desc", "Explore and validate book data.")
              )
            ),

            React.createElement("div", { className: "rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm sgwb-surface" },
              React.createElement("div", { className: "flex items-center gap-4" },
                ...
                React.createElement("button", {
                  onClick: doSearch,
                  className: "rounded-xl bg-slate-900 text-white px-5 py-3 text-sm font-extrabold hover:opacity-90 transition sgwb-primary"
...
React.createElement("div", { className: (hasSearched ? "" : "hidden ") + "mt-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm" },
  React.createElement("div", { className: "flex items-center justify-between px-6 py-5 border-b border-slate-200 dark:border-slate-800" },
    React.createElement("div", null,
      ...
      React.createElement("div", { className: "mt-1 text-xs text-slate-500 dark:text-slate-400 sgwb-muted" },
  ...
  React.createElement("div", { className: "px-6 py-5" },
    searching ? React.createElement("div", { className: "text-sm text-slate-600 dark:text-slate-300 sgwb-muted" }, t(lang, "wb.book.searching", "Searching...")) : null,
    ...
    (!searching && !searchErr && (!searchRes || searchRes.length === 0)) ? React.createElement("div", { className: "text-sm text-slate-600 dark:text-slate-300 sgwb-muted" }, t(lang, "wb.book.search.no_result", "No results.")) : null,
        ...
        searchRes.slice(0, visibleCount).map((b, idx) => {
          ...
          const inner = React.createElement("div", { className: "flex gap-4 p-4" },
            ...
            React.createElement("div", { className: "min-w-0 flex-1" },
              ...
              React.createElement("div", { className: "mt-1 text-sm text-slate-600 dark:text-slate-300 sgwb-muted truncate" }, author),
              publisher ? React.createElement("div", { className: "mt-1 text-xs text-slate-500 dark:text-slate-400 sgwb-muted truncate" }, publisher) : null,
              pubdate ? React.createElement("div", { className: "mt-1 text-xs text-slate-500 dark:text-slate-400 sgwb-muted truncate" },
                t(lang, "wb.book.pubdate", "Published"), ": ", pubdate
              ) : null,
              React.createElement("div", { className: "mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500 dark:text-slate-400 sgwb-muted" },
...
React.createElement("div", { className: (hasSearched ? "hidden " : "") + "mt-10 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm" },
              ...
              React.createElement("div", { className: "px-6 py-5" },
                loadingRecent ? React.createElement("div", { className: "text-sm text-slate-600 dark:text-slate-300 sgwb-muted" }, t(lang, "wb.book.loading", "Loading...")) : null,
                ...
                (!loadingRecent && !recentErr && recent.length === 0) ? React.createElement("div", { className: "text-sm text-slate-600 dark:text-slate-300 sgwb-muted" }, t(lang, "wb.book.empty", "No books.")) : null,
