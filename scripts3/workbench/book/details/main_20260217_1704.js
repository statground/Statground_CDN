...
async function hydrateKyoboYes24Commission(uiLang, isbn) {
  ...
  for (const tinfo of targets) {
    ...
    if (window.__WB_SHOTALK_LINK_CACHE__[ckey]) {
      ...
      a.setAttribute("data-url", window.__WB_SHOTALK_LINK_CACHE__[ckey]);
...
const marketplaceCardsHtml = marketplacesView
  ...
  .map(m => {
    ...
    const href = safeUrl; // default to fallback search URL; will be replaced by commission link when available
