...
async function set_main() {
  ...
  // URL lang (routing path)
  const seg = p.split("/").filter(Boolean);
  const urlLang = (seg && seg.length > 0 ? seg[0] : "en");

  // UI lang (selected language) - prefer base i18n current lang if provided
  const base = window.sg_workbench_i18n || {};
  const uiLang =
    (typeof base.getLang === "function" && base.getLang()) ||
    base.currentLang ||
    base.lang ||
    urlLang;
  ...
  const t = (typeof i18n.t === "function") ? i18n.t : ((_, k) => k);
  const tf = (typeof i18n.tf === "function") ? i18n.tf : ((lang, k, vars) => {
    let s = t(lang, k);
  ...
  async function fetch_detail() {
    const url = `/${urlLang}/workbench/book/ajax_detail_raw_naver/?isbn=${encodeURIComponent(isbn)}`;
  ...
  const book = j.item;
  const backUrl = `/${urlLang}/workbench/book/`;
