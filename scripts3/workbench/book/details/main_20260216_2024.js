// =====================================
// Workbench Book Detail set_main (Reactive)
// - API routing uses URL prefix lang (routeLang)
// - UI labels follow selected language (menu) via robust lang detection
...
async function set_main() {
  ...
  // URL lang (routing/API) - always safe for endpoints
  ...
  function detectUiLang() {
    // 1) base.getLang if exists
    if (typeof base.getLang === "function") {
      const x = base.getLang();
      if (x) return x;
    }
    // 2) base.currentLang / base.lang (some builds)
    if (base.currentLang) return base.currentLang;
    ...
    // 4) window.__WB_BOOK__.lang (if page provides)
    if (window.__WB_BOOK__ && window.__WB_BOOK__.lang) return window.__WB_BOOK__.lang;

    // 5) localStorage common keys (menu selection usually stored here)
    try {
  ...
  // Provide shim so other code can use it
  if (typeof base.getLang !== "function") {
    base.getLang = detectUiLang;
    window.sg_workbench_i18n = base;
  }
  ...
  function render(book, uiLang) {
    // Back link follows selected UI language (not routeLang)
  ...
  // ------------- main flow -------------
  const uiLang0 = detectUiLang();

  if (!isbn) {
    root.innerHTML = `<div class="text-sm text-rose-600">${esc(t(uiLang0, "wb.book.details.no_isbn"))}</div>`;
    return;
  }
  ...
  if (!book) {
    root.innerHTML = `<div class="text-sm text-slate-600 dark:text-slate-300">${esc(t(uiLang0, "wb.book.details.loading"))}</div>`;

    let j;
    try {
      j = await fetchDetail();
    } catch (e) {
      root.innerHTML = `<div class="text-sm text-rose-600">${esc(t(uiLang0, "wb.book.details.err"))}</div>`;
      return;
    }

    if (!j || !j.ok || !j.found || !j.item) {
      root.innerHTML = `<div class="text-sm text-rose-600">${esc(tf(uiLang0, "wb.book.details.not_found_isbn", { isbn }))}</div>`;
      return;
    }
  ...
  if (!window.__WB_BOOK_DETAIL_LANG_WATCH__) {
    window.__WB_BOOK_DETAIL_LANG_WATCH__ = { last: uiLang0 };
  }
  ...
  function rerenderIfChanged() {
    const uiLangN = detectUiLang();
    if (!uiLangN || uiLangN === state.last) return;
    state.last = uiLangN;
    render(book, uiLangN);
  }
