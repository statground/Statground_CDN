// =====================================
// Workbench Book Detail set_main (Reactive, NO recursion)
// - URL prefix lang (routeLang) has TOP priority on first render (so /it/... shows Italian)
// - After first render, UI can follow menu-selected language (storage/cookie/html lang) and re-render.
// - API routing uses routeLang to avoid 404.
// - Re-renders on language change (no re-fetch; uses cache)
// =====================================
async function set_main() {
  ...
  // URL lang (routing/API)
  const seg = path.split("/").filter(Boolean);
  const routeLang = (seg && seg.length > 0 ? seg[0] : "en");

  // ---- robust UI lang detection (NO calling base.getLang, NO overriding) ----
  ...
  function detectMenuLang() {
    // 1) <html lang="...">
    const htmlLang = (document.documentElement && document.documentElement.getAttribute("lang")) || "";
    if (htmlLang) return htmlLang;

    // 2) window.__WB_BOOK__.lang (if injected)
    if (window.__WB_BOOK__ && window.__WB_BOOK__.lang) return window.__WB_BOOK__.lang;

    // 3) localStorage common keys (menu selection often stored here)
    try {
      const keys = ["lang", "sg_lang", "statground_lang", "site_lang", "language", "wb_lang"];
    ...
    // 4) cookie common keys
    const ck = getCookie("lang") || getCookie("sg_lang") || getCookie("statground_lang") || getCookie("language");
    if (ck) return ck;
  ...
  function render(book, uiLang) {
    // Back link follows current UI language (so it goes back to that language's workbench)
  ...
  // ------------- main flow -------------
  // ✅ FIRST RENDER: URL lang wins
  let uiLang = routeLang;

  // cache book by isbn
  window.__WB_BOOK_DETAIL_CACHE__ = window.__WB_BOOK_DETAIL_CACHE__ || {};
  const cacheKey = `isbn:${isbn}`;
  let book = window.__WB_BOOK_DETAIL_CACHE__[cacheKey];

  if (!isbn) {
    root.innerHTML = `<div class="text-sm text-rose-600">${esc(t(uiLang, "wb.book.details.no_isbn"))}</div>`;
    return;
  }

  if (!book) {
    root.innerHTML = `<div class="text-sm text-slate-600 dark:text-slate-300">${esc(t(uiLang, "wb.book.details.loading"))}</div>`;

    let j;
    try {
      j = await fetchDetail();
    } catch (e) {
      root.innerHTML = `<div class="text-sm text-rose-600">${esc(t(uiLang, "wb.book.details.err"))}</div>`;
      return;
    }

    if (!j || !j.ok || !j.found || !j.item) {
      root.innerHTML = `<div class="text-sm text-rose-600">${esc(tf(uiLang, "wb.book.details.not_found_isbn", { isbn }))}</div>`;
      return;
    }

    book = j.item;
    window.__WB_BOOK_DETAIL_CACHE__[cacheKey] = book;
  }
  ...
  if (!window.__WB_BOOK_DETAIL_LANG_WATCH__) {
    window.__WB_BOOK_DETAIL_LANG_WATCH__ = { last: uiLang };
  }

  const state = window.__WB_BOOK_DETAIL_LANG_WATCH__;
  state.last = uiLang;

  function rerenderIfChanged() {
    const menuLang = detectMenuLang();
    if (!menuLang) return;

    // If user changes menu language, adopt it
    if (menuLang !== state.last) {
      state.last = menuLang;
      render(book, menuLang);
    }
  }

  // best-effort events
  window.addEventListener("sg_lang_changed", rerenderIfChanged);
  window.addEventListener("languagechange", rerenderIfChanged);
  window.addEventListener("storage", rerenderIfChanged);

  // guaranteed polling
  if (!state.timer) {
    state.timer = setInterval(rerenderIfChanged, 300);
  }
}
