...
// - Route lang (URL prefix) is used for API calls to avoid 404.
// - UI lang follows sg_workbench_i18n current selection (menu language) and re-renders on change.
// - Book data is fetched once and cached (no re-fetch on language switch).
// =====================================
async function set_main() {
  ...
  function getUiLang() {
    return (typeof base.getLang === "function" && base.getLang())
      || base.currentLang
      || base.lang
      || routeLang;
  }
  ...
  // Only start one watcher per page load
  ...
  // Event-based (best effort)
  function rerenderIfChanged() {
    const uiLangN = getUiLang();
  ...
  // Poll-based (guaranteed) - lightweight
  if (!state.timer) {
