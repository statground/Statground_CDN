...
// ------------------------------------------------------------
      ...
      (() => {
        ...
        const languages = [
          ...
          { code: 'zh-Hans', label: '中文(简体)', country: 'cn' },
          { code: 'zh-Hant', label: '中文(繁體)', country: 'tw' },
        ...
        function flagIcon(country) {
          const safeCountry = String(country || 'kr').toLowerCase().replace(/[^a-z-]/g, '') || 'kr';
          return `<span class="inline-flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-full border border-slate-200 bg-white shadow-sm"><img src="${FLAG_CDN_BASE}${safeCountry}.svg" alt="" class="h-full w-full object-cover" loading="lazy" decoding="async"></span>`;
        }
        ...
        function buildModalList(containerEl, currentLang) {
          ...
          containerEl.innerHTML = languages.map(l => {
            ...
            return `
              <button class="w-full flex items-center justify-between gap-3 px-5 py-3 text-left transition hover:bg-slate-100"
                      data-lang="${l.code}">
                <div class="flex min-w-0 items-center gap-3">
                  ${flagIcon(l.country)}
                  <div class="min-w-0">
                    <div class="font-semibold">${l.label}</div>
                    <div class="mt-0.5 text-xs text-slate-500">${l.code}</div>
                  </div>
                </div>
