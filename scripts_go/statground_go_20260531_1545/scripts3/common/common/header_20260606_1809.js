...
(function () {
  ...
  const LANG_META = {
    ...
    ja: { label: "日本語", flag: "🇯🇵" },
    "zh-Hans": { label: "中文(简体)", flag: "🇨🇳" },
    "zh-Hant": { label: "中文(繁體)", flag: "🇹🇼" },
  ...
  function langMeta(code) {
    ...
    return LANG_META[resolved] || LANG_META[String(resolved).split("-")[0]] || { label: resolved || "Language", flag: "🌐" };
  }
  ...
  function flagBadge(meta) {
    return `<span class="inline-flex h-6 w-6 shrink-0 items-center justify-center overflow-hidden rounded-full border border-slate-200 bg-white text-base leading-none shadow-sm">${esc(meta.flag)}</span>`;
  }
