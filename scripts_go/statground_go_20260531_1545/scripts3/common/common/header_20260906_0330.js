...
(function () {
  ...
  const LABELS = {
    ko: {
      ...
      data: "데이터",
      dataHub: "데이터 허브",
      localLicenses: "지방행정 인허가",
      localLicensesDesc: "업종별 인허가 자료의 항목과 기록을 확인합니다.",
      businessWorkbench: "전국 사업체·업소",
      businessWorkbenchDesc: "지역과 업종으로 인허가 기록을 찾고 분포를 살펴봅니다.",
      bookData: "도서 데이터",
      workbench: "워크벤치",
      workbenchHub: "워크벤치 홈",
      ...
      toolbox: "Toolbox",
      toolboxHub: "모든 도구 보기",
      toolboxHubDesc: "통계마당이 만드는 데이터 도구와 라이브러리를 한곳에서 살펴봅니다.",
      graflume: "Graflume",
      graflumeDesc: "이식 가능한 ChartSpec으로 통계 차트를 구성하는 Alpha 시각화 엔진입니다.",
      solidEdit: "SolidEdit",
      solidEditDesc: "로컬과 서비스 화면에 삽입할 수 있는 오프라인 우선 리치 텍스트 에디터입니다.",
      intro: "소개",
    ...
    en: {
      ...
      data: "Data",
      dataHub: "Data Hub",
      localLicenses: "Local government licensing",
      localLicensesDesc: "Browse licensing datasets and original fields by industry.",
      businessWorkbench: "Businesses & establishments",
      businessWorkbenchDesc: "Find licensing records by location and industry and explore their distribution.",
      bookData: "Book Data",
      workbench: "Workbench",
      ...
      shoppingInsight: "Shopping Price Insight",
      toolbox: "Toolbox",
      toolboxHub: "Explore all tools",
      toolboxHubDesc: "Browse data tools and libraries built by Statground.",
      graflume: "Graflume",
      graflumeDesc: "An alpha visualization engine for statistical charts built from portable ChartSpec.",
      solidEdit: "SolidEdit",
      solidEditDesc: "An offline-first rich text editor that embeds into local and hosted experiences.",
      intro: "About",
  ...
  function text(key) {
    ...
    if (context.lang === lang && context["nav_" + key]) return context["nav_" + key];
    if (key === "travelWorkbench") return "Travel & stays";
    if (key === "servicesWorkbench") return "Experts & freelance services";
    if (key === "travelWorkbenchDesc" || key === "servicesWorkbenchDesc") return "";
    ...
    const sections = [
      {
        ...
        title: text("data"),
        icon: "database",
        overview: { title: text("dataHub"), desc: text("dataDesc"), href: langPath("data/"), icon: "database" },
        items: [
          { title: text("localLicenses"), desc: text("localLicensesDesc"), href: langPath("data/local-licenses/"), icon: "database" }
        ]
      },
      {
        id: "workbench",
        ...
        items: [
          ...
          { title: text("businessWorkbench"), desc: text("businessWorkbenchDesc"), href: langPath("workbench/businesses/"), icon: "database" },
          ...
          { title: text("travelWorkbench"), desc: text("travelWorkbenchDesc"), href: langPath("workbench/travel/"), icon: "tool" },
          { title: text("servicesWorkbench"), desc: text("servicesWorkbenchDesc"), href: langPath("workbench/services/"), icon: "tool" },
      ...
      },
      {
        ...
        title: text("toolbox"),
        icon: "toolbox",
        overview: { title: text("toolboxHub"), desc: text("toolboxHubDesc"), href: "/toolbox/", icon: "toolbox" },
        items: [
          { title: text("graflume"), desc: text("graflumeDesc"), href: "/toolbox/graflume/", icon: "chart" },
          { title: text("solidEdit"), desc: text("solidEditDesc"), href: "/toolbox/solid-edit/", icon: "edit" }
        ]
      }
    ];
  ...
  function icon(name) {
    ...
    switch (name) {
      ...
      case "toolbox":
        return `<svg ${common}><path stroke-width="2" d="M4 8.5h16v10.5H4V8.5Z"></path><path stroke-width="2" d="M9 8.5V6a2 2 0 0 1 2-2h2a2 2 0 0 1 2 2v2.5M4 12h16M10 12v2h4v-2"></path></svg>`;
      case "chart":
        return `<svg ${common}><path stroke-width="2" d="M4 19h16M6 16V9M11 16V5M16 16v-4M20 7l-4 3-5-3-5 3"></path></svg>`;
      case "edit":
        return `<svg ${common}><path stroke-width="2" d="M4 20h4l11-11-4-4L4 16v4Z"></path><path stroke-width="2" d="m13.5 6.5 4 4M11 20h9"></path></svg>`;
      case "info":
  ...
  function desktopMega(section) {
    const overview = section.overview ? [
      '<div class="border-t border-slate-100 bg-slate-50 px-6 py-3 text-center">',
      `<a href="${esc(section.overview.href)}" class="inline-flex items-center gap-2 text-xs font-bold text-blue-700 hover:text-blue-900"><span>${esc(section.overview.title)}</span><span aria-hidden="true">→</span></a>`,
      '</div>'
    ].join("") : `<div class="border-t border-slate-100 bg-slate-50 px-6 py-2 text-center text-xs font-bold text-slate-500">${esc(section.title)}</div>`;
    return [
  ...
  function mobileSection(section) {
    return [
      ...
      section.overview ? `<a href="${esc(section.overview.href)}" class="flex items-start gap-3 rounded-lg bg-blue-50 px-3 py-3 text-sm text-blue-800"><span class="mt-0.5">${icon(section.overview.icon)}</span><span><span class="block font-bold">${esc(section.overview.title)}</span><span class="mt-1 block text-xs leading-5 text-blue-700">${esc(section.overview.desc)}</span></span></a>` : "",
  ...
  window.addEventListener("sg_lang_changed", function (event) {
    const next = resolveLang(event && event.detail && event.detail.lang);
    const path = window.location.pathname;
    if (next && LANG_META[next] && /^\/[^/]+\/(?:data(?:\/|$)|workbench\/businesses(?:\/|$))/.test(path)) {
      const parts = path.split("/");
      if (parts[1] !== next) {
        parts[1] = encodeURIComponent(next);
