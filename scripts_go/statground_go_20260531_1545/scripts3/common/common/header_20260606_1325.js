...
(function () {
  ...
  function menuCard(item) {
    ...
    return [
      `<a href="${esc(item.href)}"${target} class="group flex min-h-[118px] items-start gap-4 rounded-lg border border-slate-200 bg-white p-4 text-left shadow-sm hover:border-blue-300 hover:bg-blue-50 dark:border-slate-800 dark:bg-slate-900 dark:hover:border-blue-500 dark:hover:bg-slate-800">`,
      `<span class="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-slate-200 bg-slate-50 text-slate-700 group-hover:text-blue-700 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200">${icon(item.icon)}</span>`,
      '<span class="min-w-0">',
      `<span class="block text-sm font-black text-slate-950 group-hover:text-blue-700 dark:text-white">${esc(item.title)}</span>`,
      `<span class="mt-1 block text-xs leading-5 text-slate-500 dark:text-slate-300">${esc(item.desc)}</span>`,
      '</span>',
      '</a>'
    ].join("");
  }
  ...
  function desktopMega(section) {
    return [
      `<div id="sg-mega-${esc(section.id)}" data-menu-panel="${esc(section.id)}" class="hidden border-b border-slate-200 bg-white shadow-lg dark:border-slate-800 dark:bg-slate-950">`,
      '<div class="mx-auto grid max-w-7xl grid-cols-1 gap-4 px-6 py-5 md:grid-cols-2 lg:px-8">',
      section.items.map(menuCard).join(""),
      '</div>',
      `<div class="border-t border-slate-100 bg-slate-50 px-6 py-2 text-center text-xs font-bold text-slate-500 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300">${esc(section.title)}</div>`,
      '</div>'
    ].join("");
  }
  ...
  function accountLinks() {
    ...
    if (!user.name) {
      return [
        ...
        `<a href="/account/signup/" class="hidden min-h-[36px] items-center rounded-lg px-3 text-sm font-bold text-slate-700 hover:bg-slate-50 hover:text-blue-700 sm:inline-flex">${esc(text("signup"))}</a>`
      ].join("");
    }
    ...
    return [
      `<a href="/account/myinfo/" class="inline-flex min-h-[36px] items-center gap-1.5 rounded-lg px-3 text-sm font-bold text-slate-900 hover:bg-slate-50 hover:text-blue-700 dark:text-white dark:hover:bg-slate-800">${icon("user")}<span>${esc(user.name)}</span></a>`,
      role,
      admin,
      '<span class="hidden h-5 w-px bg-slate-200 sm:inline-block" aria-hidden="true"></span>',
      `<a href="/account/logout/" class="inline-flex min-h-[36px] items-center rounded-lg px-3 text-sm font-bold text-slate-700 hover:bg-slate-50 hover:text-blue-700 dark:text-slate-200 dark:hover:bg-slate-800">${esc(text("logout"))}</a>`
    ].filter(Boolean).join("");
  }

  function desktopNav(section) {
    return [
      `<button type="button" data-menu-toggle="${esc(section.id)}" aria-expanded="false" aria-controls="sg-mega-${esc(section.id)}" class="inline-flex items-center gap-1 rounded-lg px-2 py-2 text-sm font-bold text-slate-900 hover:bg-blue-50 hover:text-blue-700 dark:text-white dark:hover:bg-slate-800">`,
  ...
  function mobileSection(section) {
    return [
      '<section class="border-b border-slate-100 pb-3 dark:border-slate-800">',
      `<button type="button" data-mobile-section="${esc(section.id)}" aria-expanded="false" aria-controls="sg-mobile-section-${esc(section.id)}" class="flex min-h-[48px] w-full items-center justify-between rounded-lg px-2 text-left text-base font-black text-slate-950 hover:bg-blue-50 dark:text-white dark:hover:bg-slate-800">`,
      `<span class="inline-flex items-center gap-2"><span class="flex h-8 w-8 items-center justify-center rounded-lg bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-100">${icon(section.icon)}</span>${esc(section.title)}</span>`,
      chevron(),
      '</button>',
      `<div id="sg-mobile-section-${esc(section.id)}" class="hidden space-y-2 px-2 pb-2">`,
      section.items.map(function (item) {
        const target = item.external ? ' target="_blank" rel="noopener"' : "";
        return `<a href="${esc(item.href)}"${target} class="flex items-start gap-3 rounded-lg px-3 py-3 text-sm text-slate-700 hover:bg-slate-50 hover:text-blue-700 dark:text-slate-200 dark:hover:bg-slate-800"><span class="mt-0.5 text-slate-500">${icon(item.icon)}</span><span><span class="block font-bold">${esc(item.title)}</span><span class="mt-1 block text-xs leading-5 text-slate-500 dark:text-slate-400">${esc(item.desc)}</span></span></a>`;
      }).join(""),
      '</div>',
      '</section>'
    ].join("");
  ...
  function headerHTML() {
    ...
    return [
      '<header class="sg-header bg-white text-slate-950 shadow-sm dark:bg-slate-950 dark:text-white">',
      ...
      '<a href="/" class="flex items-center gap-3 text-xl font-black tracking-tighter text-blue-700" aria-label="Statground home">',
      '<span class="inline-flex h-9 w-9 items-center justify-center rounded-lg bg-blue-700 text-sm font-black text-white">SG</span>',
      '<span>STATGROUND</span>',
      '</a>',
      ...
      accountLinks(),
      '</div>',
      '<button type="button" id="mobile-menu-btn" class="inline-flex h-11 w-11 items-center justify-center rounded-lg text-slate-700 hover:bg-slate-100 dark:text-white dark:hover:bg-slate-800 md:hidden" aria-label="' + esc(text("openMenu")) + '" aria-controls="mobile-menu" aria-expanded="false">',
      ...
      '<button id="theme-toggle" type="button" class="inline-flex h-10 w-10 items-center justify-center rounded-full text-slate-700 hover:bg-slate-100 dark:text-white dark:hover:bg-slate-800" aria-label="' + esc(text("theme")) + '" title="' + esc(text("theme")) + '">',
      '<svg id="icon-moon" class="h-5 w-5 block" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-width="2" d="M12 3a7.5 7.5 0 0 0 9 9 9 9 0 1 1-9-9z"></path></svg>',
      '<svg id="icon-sun" class="h-5 w-5 hidden" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="4" stroke-width="2"></circle><path stroke-width="2" d="M12 2v2M12 20v2M4.93 4.93l1.41 1.41M17.66 17.66l1.41 1.41M2 12h2M20 12h2M4.93 19.07l1.41-1.41M17.66 6.34l1.41-1.41"></path></svg>',
      '</button>',
      '<button id="lang-toggle" type="button" class="inline-flex h-10 w-10 items-center justify-center rounded-full text-slate-700 hover:bg-slate-100 dark:text-white dark:hover:bg-slate-800" aria-label="' + esc(text("language")) + '" title="' + esc(text("language")) + '">',
      '<svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="10" stroke-width="2"></circle><path stroke-width="2" d="M2 12h20M12 2a15 15 0 0 1 0 20M12 2a15 15 0 0 0 0 20"></path></svg>',
      '</button>',
      '</div>',
      '</div>',
      '</nav>',
      ...
      '<a href="/" class="flex items-center gap-2 text-lg font-black tracking-tighter text-blue-700"><span class="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-blue-700 text-xs font-black text-white">SG</span>STATGROUND</a>',
      ...
      '<button id="mobile-lang-toggle" type="button" class="inline-flex h-11 w-11 items-center justify-center rounded-full text-slate-700 hover:bg-slate-100 dark:text-white dark:hover:bg-slate-800" aria-label="' + esc(text("language")) + '" title="' + esc(text("language")) + '"><svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="10" stroke-width="2"></circle><path stroke-width="2" d="M2 12h20M12 2a15 15 0 0 1 0 20M12 2a15 15 0 0 0 0 20"></path></svg></button>',
      '<button id="mobile-menu-close" type="button" class="inline-flex h-11 w-11 items-center justify-center rounded-lg text-slate-700 hover:bg-slate-100 dark:text-white dark:hover:bg-slate-800" aria-label="' + esc(text("closeMenu")) + '"><svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-width="2" d="M6 18 18 6M6 6l12 12"></path></svg></button>',
      '</div>',
      '</div>',
      '<div class="space-y-3 p-5">',
      `<a href="/" class="flex min-h-[44px] items-center gap-2 rounded-lg px-2 text-base font-black text-slate-950 hover:bg-blue-50 dark:text-white dark:hover:bg-slate-800">${icon("home")}<span>${esc(text("home"))}</span></a>`,
      sections.map(mobileSection).join(""),
      '<div class="pt-2">',
      '<div class="mb-2 px-2 text-xs font-black uppercase text-slate-400">' + esc(text("account")) + '</div>',
      '<div class="flex flex-wrap gap-2 px-2">' + accountLinks() + '</div>',
      '</div>',
      '</div>',
      '</div>',
      ...
      '<div class="w-full max-w-md overflow-hidden rounded-2xl border border-slate-200 bg-white text-slate-900 shadow-2xl dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100">',
      '<div class="flex items-center justify-between border-b border-slate-200 px-5 py-4 dark:border-slate-700"><div class="font-black tracking-tight">Language</div><button id="lang-modal-close" type="button" class="inline-flex h-10 w-10 items-center justify-center rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800" aria-label="Close language modal"><svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-width="2" d="M6 18 18 6M6 6l12 12"></path></svg></button></div>',
