(function () {
  ...
  function validateAssetRoot(value, metadata) {
    var url = new URL(sameOriginURL("./", value));
    if (
      url.search !== "" ||
    ...
    value.countries.forEach(function (country) {
      ...
      country.regions.forEach(function (region) {
        if (
          ...
          !safeString(region.isoSubdivision, 64) ||
  ...
  function fetchJSON(url, maximumBytes, assetRoot) {
  ...
  function catalogFor(state) {
    ...
    if (!catalogPromises.has(url)) {
      catalogPromises.set(
        url,
        fetchJSON(url, 6 * 1024 * 1024, state.assetRoot).then(function (value) {
  ...
  function manifestFor(state) {
    ...
    if (!manifestPromises.has(url)) {
      manifestPromises.set(
        url,
        fetchJSON(url, 1024 * 1024, state.assetRoot).then(function (manifest) {
          if (
  ...
  function loadSelectedBoundaries(state, api, manifest, selection, signal) {
    ...
    if (selection.regionIDs.length > 0) {
      jobs.push(loader.load(manifest, { level: "region", countries: selection.parentCountries }, signal).then(function (result) {
        return {
          collection: scopeCollection(api, result.collection, "isoSubdivision", selection.regionIDs),
          attribution: result.attribution,
          byteLength: result.byteLength,
          sourceIDs: result.sourceIds,
        };
  ...
  function showRetry(state, mode) {
    ...
    if (!state.retryButton) {
      ...
      button.addEventListener("click", function () {
        ...
        hideRetry(state);
        if (retryMode === "catalog") loadSelector(state);
        else if (retryMode === "apply") applySelection(state);
      });
  ...
  function applySelection(state) {
    ...
    Promise.resolve().then(function () {
      var api = runtimeAPI();
      return manifestFor(state).then(function (manifest) {
  ...
  function loadSelector(state) {
    ...
    catalogFor(state).then(function (catalog) {
      if (state.destroyed) return;
  ...
  function initializeSelector(selector) {
    ...
    try {
      ...
      if (
        selector.dataset.graflumeMapCatalogUrl !== catalogURL ||
        selector.dataset.graflumeMapManifestUrl !== manifestURL
      ) {
