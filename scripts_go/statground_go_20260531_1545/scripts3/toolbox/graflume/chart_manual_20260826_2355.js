(function () {
  ...
  function validateDataRecipe(recipe) {
    ...
    var hasDateCycleDays = Object.prototype.hasOwnProperty.call(
      recipe,
      "dateCycleDays",
    );
    var isCalendarVolume =
      recipe.family === "calendar" && recipe.profile === "volume";
    if (
      isCalendarVolume &&
      (!hasDateCycleDays ||
        !Number.isInteger(recipe.dateCycleDays) ||
        recipe.dateCycleDays < 1 ||
        recipe.dateCycleDays > 3000)
    ) {
      throw recipeError(
        "Calendar volume recipe dateCycleDays is out of range.",
      );
    }
    if (!isCalendarVolume && hasDateCycleDays) {
      throw recipeError(
        "Data recipe dateCycleDays is only valid for calendar volume.",
      );
    }
  ...
  function isoDateWithOffset(value, field, repeatIndex, dateCycleDays) {
    ...
    var dateRepeatIndex =
      dateOnly && Number.isInteger(dateCycleDays)
        ? repeatIndex % dateCycleDays
        : repeatIndex;
    var delta = dateOnly ? dateRepeatIndex * 86400000 : repeatIndex * 1000;
  ...
  function repeatVolumeRow(source, repeatIndex, steps, dateCycleDays) {
    var row = cloneRecipeValue(source, 0);
    if (!row || typeof row !== "object" || Array.isArray(row)) return row;
    var suffix = "__r" + String(repeatIndex + 1);
    Object.keys(row).forEach(function (field) {
      var value = row[field];
      var shiftedDate = isoDateWithOffset(
        value,
        field,
        repeatIndex,
        dateCycleDays,
