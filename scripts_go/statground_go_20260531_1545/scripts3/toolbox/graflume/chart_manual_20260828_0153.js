(function () {
  ...
  var manualRenderEvent = "graflume:manual-render";
  var manualRenderResultEvent = "graflume:manual-render-result";
  ...
  function setErrorState(panel, category, replaceMount) {
    ...
    signalManualRenderResult(panel, "error", category);
    syncRootState();
  }

  function signalManualRenderResult(panel, status, category) {
    var state = states.get(panel);
    if (
      !state ||
      !state.overrideToken ||
      state.overrideNotifiedGeneration === state.generation ||
    ...
    state.overrideNotifiedGeneration = state.generation;
    root.dispatchEvent(
      new window.CustomEvent(manualRenderResultEvent, {
        detail: {
          schema: "statground.graflume.manual-render-result.v1",
          panelId: panel.id,
          token: state.overrideToken,
          status: status,
          category: category || "",
        },
      }),
  ...
  function setReadyState(panel) {
    ...
    signalManualRenderResult(panel, "ready", "");
    syncRootState();
  }
  ...
  function attemptRender(state, transientState, generation) {
    ...
    try {
      ...
      var options =
        state.overrideOptions ||
        readJSON(panel, "graflumeOptionsPayloadId", "Options");
  ...
  function renderPanel(panel, transientState) {
    if (
      panel &&
      panel.dataset.graflumeMapScopeDeferred === "true" &&
      panel.dataset.graflumeMapScopeInitialReady !== "true"
    ) {
      return;
    }
    if (
    ...
    var state = existing || {
      ...
      overrideToken: "",
      overrideNotifiedGeneration: -1,
      payloadReady: false,
      lastErrorCategory: "",
    };
    states.set(panel, state);
    if (panel.dataset.graflumeMapScopePayloadId && !state.overrideOptions) {
      panel.dataset.graflumeExampleState = "loading";
      delete panel.dataset.graflumeErrorCategory;
      setStatus(
        panel,
        copy(
          "graflumeLoading",
          "Preparing the detailed map boundaries…",
          240,
        ),
        "loading",
      );
  ...
  function manualRenderPayload(detail) {
    if (
      !detail ||
      typeof detail !== "object" ||
      Array.isArray(detail) ||
      detail.schema !== "statground.graflume.manual-render.v1" ||
      typeof detail.panelId !== "string" ||
      !/^graflume-chart-map-[a-z0-9-]+$/.test(detail.panelId) ||
      typeof detail.token !== "string" ||
      !/^[a-z0-9-]{1,120}$/.test(detail.token) ||
      !Array.isArray(detail.data) ||
      detail.data.length < 1 ||
      detail.data.length > 50000 ||
      !detail.options ||
      typeof detail.options !== "object" ||
      Array.isArray(detail.options)
    ) {
      return null;
    }
    var mark = detail.options.mark;
    var markOptions = mark && typeof mark === "object" ? mark.options : null;
    var geojson =
      markOptions && typeof markOptions === "object" ? markOptions.geojson : null;
    if (
      !geojson ||
      geojson.type !== "FeatureCollection" ||
      !Array.isArray(geojson.features) ||
      geojson.features.length < 1 ||
      geojson.features.length > 50000
    ) {
      return null;
    }
    return detail;
  ...
  function handleManualRender(event) {
    if (permanentlyDestroyed) return;
    var detail = manualRenderPayload(event && event.detail);
    if (!detail) return;
    ...
    if (
      ...
      !root.contains(panel) ||
      panel.hidden ||
      !panel.dataset.graflumeMapScopePayloadId
    ) {
      return;
    }
    if (panel.dataset.graflumeMapScopeDeferred === "true") {
      panel.dataset.graflumeMapScopeInitialReady = "true";
    }
    var state = states.get(panel);
    if (!state) {
    ...
    if (!state) return;
    destroyState(state);
    state.overrideData = detail.data;
    state.overrideOptions = detail.options;
    state.overrideToken = detail.token;
    state.overrideNotifiedGeneration = -1;
    renderPanel(panel);
  }
  ...
  function handlePageHide(event) {
    ...
    root.removeEventListener(manualRenderEvent, handleManualRender);
  ...
  root.addEventListener(manualRenderEvent, handleManualRender);
