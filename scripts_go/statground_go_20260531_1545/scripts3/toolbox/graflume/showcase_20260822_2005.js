(function () {
  ...
  function rowsForCycle(cycle) {
    ...
    return baseRows.map(function (row, index) {
      return {
        ...
        target: row.target,
        actual: roundToTwo(Math.max(0, row.actual * (1 + pattern[index % pattern.length]))),
      };
    });
  }
  ...
  function buildSpec(kind) {
    ...
    if (kind === "combo") {
      spec.layers = [
        {
          id: "target",
          mark: { type: "bar", fill: "#0f766e", opacity: 0.3, cornerRadius: 7 },
          x: periodEncoding(),
          y: targetEncoding(),
        },
        {
    ...
    if (kind === "scatter") {
      spec.mark = { type: "point", fill: "#0f766e", stroke: "#ffffff", lineWidth: 2, radius: 7 };
    ...
    if (kind === "area") {
      spec.mark = { type: "area", fill: "#99f6e4", stroke: "#0f766e", lineWidth: 2.5, opacity: 0.9 };
      spec.x = periodEncoding();
      spec.y = actualEncoding();
      return spec;
    }
    ...
    spec.mark = kind === "line"
      ? { type: "line", stroke: "#0f766e", lineWidth: 3, point: true }
      : { type: "bar", fill: "#0f766e", opacity: 0.9, cornerRadius: 7 };
    spec.x = periodEncoding();
    spec.y = actualEncoding();
    return spec;
  }
  ...
  function heroSpec() {
    ...
    spec.layers = [
      {
        id: "target",
        mark: { type: "bar", fill: "#14b8a6", opacity: 0.24, cornerRadius: 6 },
        x: periodEncoding(),
        y: targetEncoding(),
      },
      {
  ...
  function galleryComboSpec() {
    ...
    spec.data = currentRows;
    spec.layers = [
      { id: "target", mark: { type: "bar", fill: "#99f6e4", cornerRadius: 5 }, x: periodEncoding(), y: targetEncoding() },
  ...
  function galleryAreaSpec() {
    ...
    spec.data = currentRows;
    spec.mark = { type: "area", fill: "#ccfbf1", stroke: "#0f766e", lineWidth: 2.5, opacity: 0.95 };
    spec.x = periodEncoding();
    spec.y = actualEncoding();
    return spec;
  }
  ...
  function handleRefreshClick() {
    ...
    try {
      ...
      showcaseCharts.forEach(function (entry) {
        entry.chart.setData(currentRows);
      });
      updateTable();
  ...
  function cleanup() {
    ...
    elements.kind.removeEventListener("click", handleKindClick);
