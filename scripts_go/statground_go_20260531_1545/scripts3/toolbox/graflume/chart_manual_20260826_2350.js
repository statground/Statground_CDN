(function () {
  ...
  function surfaceVolumeSide(targetRows) {
    var side = Math.max(1, Math.floor(Math.sqrt(targetRows)));
    while ((side + 1) * (side + 1) <= targetRows) side += 1;
    while (side * side > targetRows) side -= 1;
    return side;
  }
  ...
  function resolveSurfaceRecipe(recipe, base) {
    ...
    if (recipe.profile === "range") {
      result.z = scaledNumericArray(source.z);
      result.values = scaledNumericArray(source.values);
    } else if (recipe.profile === "structure") {
      result.z = source.z.slice().reverse();
      result.values = source.values.slice().reverse();
    } else {
      var side = surfaceVolumeSide(recipe.targetRows);
      var cellCount = side * side;
      var z = new Array(cellCount);
      var values = new Array(cellCount);
      for (var row = 0; row < side; row += 1) {
        var sourceRow =
          side === 1 ? 0 : Math.round((row * (source.rows - 1)) / (side - 1));
        for (var column = 0; column < side; column += 1) {
          var sourceColumn =
            side === 1
              ? 0
              : Math.round((column * (source.columns - 1)) / (side - 1));
          var sourceIndex = sourceRow * source.columns + sourceColumn;
          var targetIndex = row * side + column;
      ...
      result.rows = side;
      result.columns = side;
      result.x = interpolatedAxis(source.x, side);
      result.y = interpolatedAxis(source.y, side);
      result.z = z;
  ...
  function validateVolumeData(value) {
    ...
    var dimensions = value.dimensions.map(function (entry) {
      return positiveInteger(entry, "Volume dimension");
    });
  ...
  function volumeCubeSide(targetRows) {
    var side = Math.max(1, Math.min(64, Math.floor(Math.cbrt(targetRows))));
    while (side < 64 && Math.pow(side + 1, 3) <= targetRows) side += 1;
    while (Math.pow(side, 3) > targetRows) side -= 1;
  ...
  function resolveSpatialVolumeRecipe(recipe, base) {
    ...
    if (recipe.profile === "range") {
      ...
      var side = volumeCubeSide(recipe.targetRows);
      var voxelCount = Math.pow(side, 3);
      var values = new Array(voxelCount);
      for (var index = 0; index < voxelCount; index += 1) {
        values[index] = source.values[index % source.values.length];
      }
      result.dimensions = [side, side, side];
      result.values = values;
    }
  ...
  function validateSpatialVectorData(value) {
    ...
    if (!Array.isArray(value.labels) || !Array.isArray(value.colors)) {
      throw recipeError("Spatial vector labels and colors must be arrays.");
    }
    var length = origins.length;
    if (
      vectors.length !== length ||
      value.labels.length !== length ||
      value.colors.length !== length
    ) {
  ...
  function resolveSpatialVectorRecipe(recipe, base) {
    ...
    var result = cloneRecipeValue(base, 0);
    var fields = ["origins", "vectors", "labels", "colors"];
    if (recipe.profile === "range") {
      result.vectors = source.vectors.map(function (vector, index) {
        var factor = dataRecipeFactors[index % dataRecipeFactors.length];
      ...
      for (var index = 0; index < recipe.targetRows; index += 1) {
        var sourceIndex = index % source.origins.length;
        var repeatBatch = Math.floor(index / source.origins.length);
        result.origins[index] = cloneRecipeValue(
          source.origins[sourceIndex],
          0,
        );
        result.vectors[index] = cloneRecipeValue(
          source.vectors[sourceIndex],
          0,
        );
        result.labels[index] =
  ...
  function resolveDataRecipe(value, scope) {
    ...
    if (recipe.family === "surface") {
      return resolveSurfaceRecipe(recipe, base);
    }
    if (recipe.family === "volume") {
      return resolveSpatialVolumeRecipe(recipe, base);
    }
    if (recipe.family === "spatial-vector") {
      return resolveSpatialVectorRecipe(recipe, base);
    }
    if (!Array.isArray(base)) {
      throw recipeError("Object data recipes require a supported family.");
    }
    if (recipe.profile === "range") return resolveRangeRecipe(recipe, base);
    if (recipe.profile === "structure") {
      return resolveStructureRecipe(base);
    }
    return resolveVolumeRecipe(recipe, base);
  }
