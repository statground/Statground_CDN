(function () {
    ...
    var profiles = documentValue.profiles.map(function (entry) {
      ...
      if (
        ...
        capabilities.some(function (value) {
          return (
            typeof value !== "string" ||
            !adaptiveCapabilityPattern.test(value)
          );
        })
      ) {
  ...
  function tableVisible(dataPanel) {
    return dataPanel.tagName === "DETAILS"
      ? dataPanel.open
      : !dataPanel.hidden;
  }
  ...
  function activeTableControl(panel) {
    return (
      panel.querySelector(
        "[data-graflume-table-toggle-proxy]:not([hidden])",
  ...
  function installTableControl(panel) {
    ...
    var existing = panel.querySelector(
      "[data-graflume-table-toggle-proxy]",
    );
    if (existing) return true;
    var toolbar =
      panel.querySelector("[data-graflume-controls-strip]") ||
      panel.querySelector("[data-graflume-spatial-controls]");
  ...
  function observeTableControl(panel, state) {
    if (state.tableControlObserver || typeof window.MutationObserver !== "function")
      return;
    var mount = elementFor(panel, "graflumeMountId");
    if (!mount) return;
  ...
  function initializeHostControls(panel, state) {
    var tableInstalled = installTableControl(panel);
    ...
    if (tableToggle && dataPanel) {
      tableToggle.hidden = tableInstalled;
      ...
      var closeButton = dataPanel.querySelector(
        "[data-graflume-table-close]",
      );
      if (closeButton) {
  ...
  function releaseStateResources(state) {
    ...
    if (state.tableControlObserver) {
      try {
        ...
        reportCleanupError("Graflume table-control observer cleanup failed.", error);
      }
    }
    ...
    try {
      ...
      if (state.materializationPlan) {
        ...
        var adaptive =
          ...
          typeof options.create.adaptive === "object"
            ? options.create.adaptive
            : {};
        var environment =
  ...
  function activateGroup(group, panel, focusTab, updateHash) {
    ...
    if (!confirmPanelActivation(panel)) {
      ...
      var retainedTab = retainedPanel ? tabForPanel(group, retainedPanel) : null;
      if (!retainedTab) {
  ...
  function bindAdaptiveSection(section) {
    ...
    var selectedProfile = adaptiveRegistry.profileByID[
      adaptiveRegistry.defaultProfile
    ];
    var hash = window.location.hash.slice(1);
  ...
  function initialize() {
    ...
    try {
      ...
      if (window.console && typeof window.console.error === "function") {
        window.console.error("Graflume adaptive-device registry failed.", error);
      }
    }
    alwaysPanels.forEach(function (panel) {
      panel.hidden = false;
