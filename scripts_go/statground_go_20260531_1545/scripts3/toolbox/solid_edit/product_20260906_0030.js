(function solidEditProduct(global) {
  ...
  function initProduct(document, window) {
    ...
    var outputCode = byID("solid-edit-demo-output-code") || output;
    ...
    var defaults = {
      loading: "Loading the editor…", readyFull: "Full editor ready", readyMini: "Mini editor ready",
    ...
    var pending = null;
    var switching = false;
    var codeCache = new WeakMap();

    function clearHighlight(element) {
      Array.from(element.classList).forEach(function (name) {
        if (name === "hljs" || /^language-/.test(name)) element.classList.remove(name);
      });
      element.removeAttribute("data-highlighted");
    }
    function highlightCode(element, value, language) {
      if (!element) return;
      var sourceText = String(value == null ? "" : value);
      var highlighter = window.hljs;
      var previous = codeCache.get(element);
      if (previous && previous.source === sourceText && previous.language === language && previous.highlighter === highlighter) return;
      // Always create a text node first; only the pinned highlighter may add token spans.
      text(element, sourceText);
      clearHighlight(element);
      codeCache.set(element, { source: sourceText, language: language, highlighter: highlighter });
      // Large image/SVG exports remain complete and readable without costly tokenization.
      if (!sourceText || sourceText.length > 262144 || !/^(xml|markdown|javascript)$/.test(language)) return;
      try {
        if (!highlighter || typeof highlighter.highlightElement !== "function" ||
            typeof highlighter.getLanguage !== "function" || !highlighter.getLanguage(language)) return;
    ...
    function highlightExamples() {
      Array.prototype.forEach.call(document.querySelectorAll("[data-solid-edit-code]"), function (element) {
        highlightCode(element, element.textContent, element.dataset.solidEditCode);
      });
    }
    ...
    function renderOutput() {
      ...
      try {
        ...
        highlightCode(outputCode, value || copy.outputEmpty, value ? (outputMode === "html" ? "xml" : "markdown") : "");
      } catch (_) { highlightCode(outputCode, copy.runtimeError, ""); }
    ...
    function syncControls() {
      ...
      options.push("  toolbarSize: " + JSON.stringify(mode));
      highlightCode(code, "const editor = window.mountContentEditor(\"#content\", {\n" + options.join(",\n") + "\n});", "javascript");
    }
