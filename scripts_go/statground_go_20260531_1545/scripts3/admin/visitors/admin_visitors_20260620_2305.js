(function () {
  ...
  var fallbackCacheKey = "statground.admin.visits.dashboard.v1";
  ...
  function rememberDashboard(data) {
    ...
    try {
      window.sessionStorage.setItem(fallbackCacheKey, JSON.stringify(data));
    } catch (error) {
      // Storage can be unavailable in privacy modes. The server stale cache still protects the page.
    }
  }
  ...
  function cachedDashboard() {
    ...
    try {
      var raw = window.sessionStorage.getItem(fallbackCacheKey);
  ...
  function fetchDashboard(root, attempt) {
    ...
    return fetch(url, { credentials: "same-origin", headers: { "Accept": "application/json" } })
      ...
      .then(function (payload) {
        if (!payload || payload.ok !== true || !payload.dashboard) {
          throw new Error("dashboard payload unavailable");
        }
