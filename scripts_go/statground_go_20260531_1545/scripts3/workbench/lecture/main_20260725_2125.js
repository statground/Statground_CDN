(function () {
  ...
  const cdnRoot = (() => {
    ...
    const idx = scriptURL.indexOf(marker);
    if (idx > 0) return scriptURL.slice(0, idx + 1);
    return "/";
  })();

  function cdnAsset(path) {
    return cdnRoot + String(path || "").replace(/^\/+/, "");
  }
  ...
  function affiliateCardsHTML(lang, item) {
    const inflearnURL = esc(inflearnAffiliateHref(lang, item));
    const inflearnLogo = esc(cdnAsset("images/data/lecture/inflearn.svg"));
    const adpickLogo = esc(cdnAsset("images/common/affiliates/adpick.png"));
    const openLabel = esc(t(lang, "openOriginal"));

    return `
      <div class="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
        <a href="${inflearnURL}" target="_blank" rel="nofollow sponsored noopener noreferrer"
           class="rounded-xl border border-slate-200 bg-white p-3 flex min-h-[148px] flex-col items-center justify-center gap-2 transition hover:bg-slate-50 dark:border-slate-800 dark:bg-slate-900 dark:hover:bg-slate-800">
          <div class="flex h-12 w-32 items-center justify-center rounded-lg border border-slate-200 bg-slate-50 px-3 dark:border-slate-800 dark:bg-slate-950">
            <img src="${inflearnLogo}" alt="Inflearn" width="104" height="22" class="max-h-8 w-full object-contain" loading="lazy" decoding="async" />
          </div>
          <div class="text-center text-sm font-extrabold text-slate-900 dark:text-white">Inflearn</div>
          <div class="rounded-md bg-slate-900 px-2 py-1 text-xs font-semibold text-white dark:bg-white dark:text-slate-900">${openLabel}</div>
        </a>
        <div class="sg-adpick-card rounded-xl border border-slate-200 bg-white p-3 flex min-h-[148px] flex-col items-center justify-center dark:border-slate-800 dark:bg-slate-900">
          <img src="${adpickLogo}"
               alt="이글의 홍보링크로 구매하시면 수수료를 제공받습니다." width="125" height="125"
               class="h-[125px] w-[125px] object-contain" loading="lazy" decoding="async" />
        </div>
      </div>
    `;
  }
  ...
  function syncLangToURL(lang) {
    const raw = String(lang || "").trim();
    if (!raw) return false;
    const langCode = displayLang(raw);
          ...
          <article class="rounded-lg border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div class="grid grid-cols-1 gap-6 md:grid-cols-12">
              ...
              <div class="md:col-span-8">
                ...
                ${affiliateCardsHTML(langCode, item)}
              </div>
            </div>
  ...
  async function render(langOverride) {
