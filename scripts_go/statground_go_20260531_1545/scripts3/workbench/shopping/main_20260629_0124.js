(function () {
  ...
  const dict = {
    ko: {
      ...
      categoryDeckDesc: "먼저 카테고리를 선택하면 해당 범주의 관측 상품 후보가 아래에 표시됩니다. 위 차트는 선택값과 무관하게 전체 카테고리 시장 스냅샷을 유지합니다.",
      viewCategory: "상품 보기",
      selectedCategory: "선택 카테고리",
      loadingCategory: "카테고리 상품을 불러오는 중...",
      categoryProducts: "카테고리 상품 후보",
      categoryHint: "카테고리를 선택하면 상품 후보가 표시됩니다.",
      categoryAllCharts: "전체 카테고리 기준 차트",
      categoryEmpty: "표시할 카테고리 상품 후보가 없습니다."
    },
    en: {
      back: "Workbench",
      title: "Shopping Price Insight",
      ...
      categoryDeckDesc: "Select a category first to show observed product candidates below. The charts above remain a market snapshot across all categories, independent of the selected category.",
      viewCategory: "View products",
      selectedCategory: "Selected category",
      loadingCategory: "Loading category products...",
      categoryProducts: "Category product candidates",
      categoryHint: "Select a category to show product candidates.",
      categoryAllCharts: "Charts across all categories",
      categoryEmpty: "No displayable product candidates for this category."
    }
  };
  ...
  function renderInsightCharts(lang, radar, finder) {
    const radarCandidates = (radar && radar.deal_candidates) || [];
    ...
    const categories = radarCategories.length ? radarCategories : ((finder && finder.category_options) || []);
    const radarSellers = (radar && radar.seller_insights) || [];
    const sellers = radarSellers.length ? radarSellers : ((finder && finder.seller_opportunities) || []);
    ...
    paintChart("sg-shopping-chart-category", lang, categories.length ? {
      ...
      series: [
        { type: "bar", name: t(lang, "lowPriceShare"), data: categories.map((row) => Number(row.low_price_percent || 0)), barMaxWidth: 24, itemStyle: { borderRadius: [5, 5, 0, 0] } },
        { type: "line", name: t(lang, "discount"), data: categories.map((row) => Number(row.discounted_percent || 0)), smooth: true, symbolSize: 7 }
      ]
    } : null);
    ...
    paintChart("sg-shopping-chart-seller", lang, sellers.length ? {
      ...
      series: [{
        ...
        data: sellers.map((row) => Math.round((Number(row.low_price_percent || 0) * 1.3 + Number(row.discounted_percent || 0) + Math.min(Number(row.product_count || 0), 30)) * 10) / 10),
        barMaxWidth: 22,
  ...
  function render(langOverride) {
    ...
    function showApp(payload) {
      ...
      appEl.innerHTML = [
        ...
        renderChartsShell(lang),
        '</div>',
        '<div id="sg-shopping-category-deck">',
        renderCategoryDeck(lang, radar, window.__statgroundSelectedCategory || ""),
        '</div>',
        '<div id="sg-shopping-category-result" class="mt-6">',
        renderCategoryResult(lang, null, ""),
        '</div>'
      ].join("");
