(function () {
  ...
  function defaultCategoryFromRadar(radar) {
    const rows = categoryRows(radar);
    return rows.length && rows[0].source_category ? rows[0].source_category : "";
  }

  function isRootCategoryView() {
    return analysisModeFromURL() !== "keyword" && compareSlugsFromURL().length <= 1 && !currentCategorySlug();
  }
  ...
  function mergeKeywordSearchResult(radar, result) {
    ...
    const keywords = ((payload && payload.keywords) || []).filter(visibleKeywordRow);
    const categoryKeywords = ((payload && payload.category_keywords) || []).filter(visibleKeywordRow);
    ...
    next.keywords = mergeRowsByKey((radar && radar.keywords) || [], keywords, (row) => keywordKey(row && row.keyword));
    next.category_keywords = mergeRowsByKey((radar && radar.category_keywords) || [], categoryKeywords, (row) => normalizedCategorySlug(row && row.source_category) + "::" + keywordKey(row && row.keyword));
    next.products = uniqueShoppingItems(products.concat((radar && radar.products) || []));
    next.deal_candidates = (radar && radar.deal_candidates) || [];
    next.price_range_slices = mergeKeywordSearchIntoSlices((radar && radar.price_range_slices) || [], keywords, categoryKeywords, products);
    return next;
  }
  ...
  function filterPriceBandsByRange(bands, rawRange) {
    const range = normalizePriceRange(rawRange);
    if (!range) return (bands || []).slice();
    const rows = (bands || []).filter((band) => rangeOverlaps(band && band.min_price_krw, band && band.max_price_krw, range));
    const total = rows.reduce((sum, row) => sum + aggregateRowWeight(row), 0);
    return rows.map((row) => Object.assign({}, row, {
  ...
  function priceFilteredRadar(radar, rawRange) {
    ...
    const bands = filterPriceBandsByRange((radar && radar.price_bands) || [], range);
  ...
  function renderPriceBandCriteriaStage(lang, radar, selectedCategory, selectedKeywords, mode) {
    ...
    const sourceTotal = weightedCriteriaCount(sourceItems) || sourceItems.length;
    ...
    const body = [
      ...
      pricePresets.map((preset) => {
        ...
        const count = preset.key === "all" ? sourceTotal : weightedCriteriaCount(filterItemsByPriceRange(sourceItems, preset));
  ...
  function renderResearchDashboard(lang, radar, categoryAxisRadar) {
    ...
    if (!selected) {
      return renderCategoryAxisStage(lang, axisRadar, "", []);
    }
    const analysisRadar = priceFilteredRadar(radar, currentPriceRange());
    return [
  ...
  function fetchKeywordMartResult(lang, query, category) {
    return fetchJSONWithRetry(apiURL(lang, "ajax_keyword_search_gmarket", keywordMartSearchParams(query, category)), 2)
      .then((res) => {
        ...
        return res.json && res.json.result ? res.json.result : res.json;
      });
  }
  ...
  function applyKeywordMartResult(lang, radar, appEl, result, mode) {
    ...
    window.__statgroundKeywordSearchQuery = result && result.query ? result.query : window.__statgroundKeywordSearchQuery;
    if (mode === "keyword") {
  ...
  function setupKeywordMartSearch(lang, radar, appEl, mode) {
    ...
    form.addEventListener("submit", function (event) {
      ...
      if (!query) return;
      const category = mode === "category" ? (window.__statgroundSelectedMarketCategory || "") : "";
      ...
      fetchKeywordMartResult(lang, query, category)
        .then((result) => {
          ...
          if (!applied && button) {
            button.textContent = t(lang, "keywordSearchNoResult");
            window.setTimeout(() => {
              button.textContent = originalLabel || t(lang, "keywordSearchButton");
              button.disabled = false;
            }, 900);
          }
        })
        .catch(() => {
          if (button) button.textContent = t(lang, "keywordSearchNoResult");
          window.setTimeout(() => {
            if (button) {
              button.textContent = originalLabel || t(lang, "keywordSearchButton");
              button.disabled = false;
  ...
  function maybeFetchKeywordMartFallback(lang, radar, appEl, selectedKeywords, currentRows) {
    const selected = (selectedKeywords || []).filter(Boolean);
    if (!selected.length || (currentRows || []).length) return;
    ...
    fetchKeywordMartResult(lang, selected.join(" "), "")
      .then((result) => {
        applyKeywordMartResult(lang, radar, appEl, result, "keyword");
      })
      .catch(() => {});
  }
  ...
  function render(langOverride) {
    ...
    function showApp(payload) {
      ...
      if (isRootCategoryView()) {
        const defaultCategory = defaultCategoryFromRadar(radar);
        if (defaultCategory) {
          fetchJSONWithRetry(apiURL(lang, "ajax_radar_gmarket", { category: defaultCategory }), 3)
            .then((res) => {
              const categoryRadar = res && res.ok && res.json && res.json.radar ? res.json.radar : radar;
              renderLoadedRadar(lang, categoryRadar, appEl, { baseRadar: radar, defaultCategory: defaultCategory });
            })
            .catch(function () {
              renderLoadedRadar(lang, radar, appEl, { baseRadar: radar, defaultCategory: defaultCategory });
            });
          return;
        }
      }
