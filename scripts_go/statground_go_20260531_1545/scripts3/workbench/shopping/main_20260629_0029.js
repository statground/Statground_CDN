(function () {
  ...
  const dict = {
    ko: {
      ...
      notice: "상품명, 이미지, 상세설명, 판매자, 브랜드, raw payload는 표시하지 않습니다. 표시 가격은 수집 시점 관측값이며 배송비와 옵션 총액은 외부몰에서 최종 확인해야 합니다.",
      budget: "예산",
      category: "카테고리",
      query: "찾는 것",
  ...
  function renderWatchlist(lang) {
    ...
    return [
      ...
      rows.map((item) => [
        ...
        '<div><div class="font-black text-slate-900">' + esc(item.product_label || item.product_code) + '</div><div class="mt-1 text-slate-500">' + esc(item.source_category || "") + ' · ' + esc(krw(item.price_krw || 0)) + '</div></div>',
