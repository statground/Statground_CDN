(function () {
  ...
  function keywordProducts(radar, row, limit) {
    ...
    return scored.slice(0, limit || 3).map((entry) => entry.item);
  }
  ...
  function renderKeywordProducts(lang, radar, rows) {
    ...
    const visible = (rows || []).map((row) => ({ row: row, products: keywordProducts(radar, row, explicitKeyword ? 6 : 3) })).filter((entry) => entry.products.length);
  ...
  function renderLoadedKeywordRadar(lang, radar, appEl) {
    ...
    bindWatchButtons(lang, rows.reduce((items, row) => items.concat(keywordProducts(radar, row, selectedKeywords.length ? 6 : 3)), []));
  }
