(function () {
  ...
  function normalizeCategoryBenchmarkRows(rows) {
    ...
    return STANDARD_CATEGORIES.map((standard) => {
      ...
      const row = bucket.row;
      row.p25_price_krw = weightedAverage(bucket.p25, bucket.weight);
      row.median_price_krw = weightedAverage(bucket.p50, bucket.weight);
      row.p75_price_krw = weightedAverage(bucket.p75, bucket.weight);
      ...
      row.demand_score = weightedAverage(bucket.demand, bucket.weight);
      row.competition_score = weightedAverage(bucket.competition, bucket.weight);
      row.price_gap_score = weightedAverage(bucket.priceGap, bucket.weight);
      row.opportunity_score = weightedAverage(bucket.opportunity, bucket.weight);
      return row;
    }).filter(Boolean);
  }
