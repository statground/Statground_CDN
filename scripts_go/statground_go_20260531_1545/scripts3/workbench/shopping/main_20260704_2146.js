(function () {
  ...
  const dict = {
    ko: {
      ...
      notice: "이 화면은 상품 상세를 복제하지 않고 플랫폼별 관측 상품과 카테고리·키워드·가격 범위의 집계 지표를 보여줍니다. 표시 가격과 반응 지표는 측정 시각의 관측값이며 실제 구매·판매 판단 전에는 외부몰과 제휴 정책을 확인해야 합니다.",
      budget: "예산",
      category: "카테고리",
      query: "찾는 것",
      ...
      categoryStageDesc: "여러 판매처의 세부 카테고리를 공통 기준으로 묶어 비교합니다. 카테고리는 분석을 시작하는 기준이고, 키워드는 여러 카테고리에 함께 걸칠 수 있습니다.",
    ...
    en: {
      ...
      notice: "This view does not replicate product detail pages. It shows provider-aware observed products plus aggregate metrics by category, keyword, and price range. Prices and reaction signals are observations at measurement time; verify the external mall and affiliate policy before buying or selling decisions.",
      budget: "Budget",
      category: "Category",
      query: "Need",
      ...
      categoryStageDesc: "Provider-specific categories are grouped into shared comparison buckets. Use the category as a starting point; keywords can still span multiple categories.",
  ...
  function chartBox(id, title) {
    ...
    return [
      ...
      '</div>',
      '<div id="' + esc(id) + '" class="h-[320px] w-full rounded-lg bg-slate-50">' + chartFallbackHTML(lang, "chartPreparing") + '</div>',
      '</section>'
    ].join("");
  }
  ...
  function renderSnapshot(lang, radar) {
    ...
    return [
      ...
      statCard(t(lang, "kpiPolicy"), t(lang, "kpiPolicySub"), t(lang, "affiliateDisclosure")),
      '</section>'
    ].join("");
  }
  ...
  function renderKeywordAxisStage(lang, radar, selectedCategory, selectedKeywords) {
    ...
    const body = [
      ...
      '</div>',
      '</div>',
      '<p class="mb-2 text-xs font-black uppercase text-slate-500">' + esc(t(lang, "keywordPickHint")) + '</p>',
      ...
      rows.map((row) => {
        ...
        const button = [
          '<button type="submit" data-top-keyword="' + esc(row.keyword || "") + '" aria-label="' + esc(t(lang, "submitKeyword") + " · " + (row.keyword || "")) + '" aria-pressed="' + (active ? "true" : "false") + '" class="group flex min-h-[132px] w-full flex-col rounded-lg border p-3 text-left transition focus:outline-none focus:ring-2 focus:ring-blue-300 ' + (active ? "border-slate-950 bg-slate-950 text-white shadow-md" : "border-slate-200 bg-white hover:border-blue-300 hover:bg-blue-50") + '">',
          '<div class="flex items-start justify-between gap-3">',
          '<div><div class="text-[10px] font-black uppercase ' + (active ? "text-white/65" : "text-slate-500") + '">' + esc(t(lang, "submitKeyword")) + '</div><h3 class="mt-0.5 text-sm font-black leading-4 ' + (active ? "text-white" : "text-slate-950") + '">#' + esc(row.keyword || "-") + '</h3></div>',
          '<span class="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs ' + (active ? "bg-white text-slate-950" : "bg-slate-100 text-slate-700 group-hover:bg-white group-hover:text-blue-700") + '">#</span>',
          '</div>',
          '<div class="mt-3 grid grid-cols-3 gap-1.5 text-[11px] font-bold">',
          ...
          '</div>',
          '<div class="mt-2 flex flex-wrap gap-1">' + categorySummary + crossBadge + '</div>',
          '<div class="mt-auto pt-2 text-[11px] font-black ' + (active ? "text-white/80" : "text-blue-700") + '">' + esc(t(lang, "openKeywordView")) + '</div>',
          '</button>'
        ].join("");
  ...
  function renderKeywordProducts(lang, radar, rows) {
    ...
    return [
      ...
      visible.map((entry) => [
        ...
        '</div>',
        '<div class="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5" data-keyword-products-grid>',
  ...
  function productCard(lang, item) {
    ...
    return [
      '<article class="flex h-full flex-col rounded-lg border border-slate-200 bg-white p-3 shadow-sm">',
      ...
      '<h3 class="text-xs font-black leading-4 text-slate-950">' + esc(label) + '</h3>',
      ...
      '</div>',
      '</div>',
      '<span class="rounded-full bg-emerald-50 px-2 py-0.5 text-[11px] font-black text-emerald-700">' + esc(number(item && (item.deal_confidence_score || item.radar_score) || 0)) + '</span>',
      '</div>',
      ...
      key ? '<button type="button" data-watch-code="' + esc(key) + '" class="min-h-[34px] rounded-lg border border-slate-300 px-2.5 py-1.5 text-[11px] font-black text-slate-700 hover:border-slate-500">' + esc(isWatched ? t(lang, "savedWatch") : t(lang, "saveWatch")) + '</button>' : '',
      url ? '<a href="' + esc(url) + '" target="_blank" rel="noopener noreferrer" class="inline-flex min-h-[34px] items-center justify-center rounded-lg bg-slate-900 px-2.5 py-1.5 text-[11px] font-black text-white hover:bg-slate-700">' + esc(t(lang, "source")) + '</a>' : '',
      '</div>',
      '</article>'
    ].join("");
  }
  ...
  function renderCandidates(lang, items) {
    ...
    return '<section><h2 class="mb-3 text-lg font-black text-slate-950">' + esc(t(lang, "candidates")) + '</h2><div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">' + items.map((item) => productCard(lang, item)).join("") + '</div></section>';
  }
  ...
  function renderObservedProducts(lang, radar, selectedCategory) {
    ...
    return '<section><h2 class="mb-3 text-lg font-black text-slate-950">' + esc(selectedCategory + " · " + t(lang, "categoryObservedProducts")) + '</h2><div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">' + items.map((item) => productCard(lang, item)).join("") + '</div></section>';
  }
