(function () {
  ...
  function keywordAxisRows(radar, selectedCategory, selectedKeywords) {
    ...
    if (activeKeywordSearchQuery(radar)) rows = keywordSearchFocusedRows(radar, rows);
  ...
  function baseKeywordRows(radar) {
    ...
    return activeKeywordSearchQuery(radar) ? keywordSearchFocusedRows(radar, rows) : rows;
  }
  ...
  function keywordRowMatchesSearchQuery(row, query) {
    ...
    if (!q) return false;
    const key = keywordKey(row && row.keyword);
    if (key && (key.indexOf(q) >= 0 || q.indexOf(key) >= 0)) return true;
    const text = keywordKey([row && row.keyword, row && row.source_category].filter(Boolean).join(" "));
    return !!text && text.indexOf(q) >= 0;
  }

  function productMatchesSearchQuery(item, query) {
    const q = keywordKey(query);
    if (!q) return false;
    const text = keywordKey([
      productLabel(item),
      item && item.brand,
      item && item.seller,
  ...
  function renderKeywordDiscovery(lang, radar, selectedCategory) {
    ...
    rows = (searchQuery ? keywordSearchFocusedRows(radar, rows) : rows)
      .filter(visibleKeywordRow)
      .slice()
      .sort((a, b) => {
        if (searchQuery) {
  ...
  function renderObservedProducts(lang, radar, selectedCategory) {
    ...
    const title = (selectedCategory || t(lang, "allMarket")) + " · " + t(lang, "categoryObservedProducts");
