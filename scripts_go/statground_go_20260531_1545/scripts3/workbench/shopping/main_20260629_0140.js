(function () {
  ...
  function renderKeywordDiscovery(lang, radar) {
    const rows = (radar && radar.keywords) || [];
    if (!rows.length) return "";
    return [
      '<section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">',
      '<h2 class="text-lg font-black text-slate-950">' + esc(t(lang, "keywordDiscovery")) + '</h2>',
  ...
  function renderInsightCharts(lang, radar, finder) {
    const crossPoints = ((radar && radar.category_keywords) || []).map((item) => ({
      source_category: item.source_category || "",
    ...
    const radarCandidates = crossPoints.length ? crossPoints : ((radar && radar.deal_candidates) || []);
    ...
    const sellers = radarCategories.length ? radarCategories : (radarSellers.length ? radarSellers : ((finder && finder.seller_opportunities) || []));
  ...
  function bindResearchDashboard(lang, radar, appEl) {
    ...
    appEl.querySelectorAll("[data-market-category]").forEach((btn) => {
      btn.addEventListener("click", function () {
        ...
        window.__statgroundSelectedMarketCategory = window.__statgroundSelectedMarketCategory === category ? "" : category;
