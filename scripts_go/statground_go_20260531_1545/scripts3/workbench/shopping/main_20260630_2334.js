(function () {
  ...
  const dict = {
    ...
    en: {
      ...
      marketStageTitle: "1. Start from the whole market",
      marketStageDesc: "First set the baseline with observed product volume, price bands, discount, and low-price signals.",
      categoryStageTitle: "2. Choose a category lens",
      ...
      keywordStageTitle: "3. Narrow through the keyword axis",
      ...
      segmentStageTitle: "4. Read the category × keyword cross point",
      segmentStageDesc: "At the intersection, compare price percentiles, demand proxy, competition, price gap, and opportunity.",
      evidenceStageTitle: "5. Check product evidence and boundaries",
  ...
  function categoryHref(lang, value) {
    ...
    return shoppingBasePath(lang) + (slug ? encodeURIComponent(slug) + "/" : "");
  }
  ...
  function selectedKeywordsFromURL() {
    try {
      ...
      const values = repeated.concat(joined ? joined.split(",") : []);
      const seen = {};
      const rows = [];
      values.forEach((value) => {
        const text = String(value || "").trim();
  ...
  function keywordHref(lang, keywords) {
    ...
    (keywords || []).forEach((keyword) => {
      ...
      if (!text || !key || seen[key]) return;
      seen[key] = true;
      query.append("keyword", text);
    });
    return shoppingBasePath(lang) + "?" + query.toString();
  }
  ...
  function currentCategorySlug() {
    try {
      ...
      const slug = parts[workbenchIndex + 2] || "";
  ...
  function renderAnalysisPath(lang, mode, selectedCategory, selectedKeywords) {
    ...
    const activeIndex = mode === "keyword" ? (keywordSelected ? 3 : 2) : (selectedCategory ? 3 : 1);
    const steps = [
      { key: "pathMarket", active: activeIndex >= 1 },
      { key: "pathCategory", active: mode !== "keyword" && activeIndex >= 2 },
      { key: "pathKeyword", active: mode === "keyword" || activeIndex >= 2 },
      { key: "pathSegment", active: activeIndex >= 3 },
      { key: "pathEvidence", active: activeIndex >= 3 }
    ];
    return [
  ...
  function categoryAxisTile(lang, item, active, rank) {
    ...
    return [
      '<a href="' + esc(categoryHref(lang, name)) + '" data-market-category="' + esc(name) + '" class="group flex min-h-[176px] flex-col rounded-lg border p-3 transition ' + (active ? "border-slate-950 bg-slate-950 text-white shadow-md" : "border-slate-200 bg-white hover:border-blue-300 hover:bg-blue-50") + '">',
      '<div class="flex items-start justify-between gap-3">',
  ...
  function renderCategoryAxisStage(lang, radar, selectedCategory, selectedSlugs) {
    ...
    const body = [
      ...
      '</div>',
      selectedCategory ? '<div class="mt-3"><a href="' + esc(shoppingBasePath(lang)) + '" class="inline-flex min-h-[36px] items-center rounded-full border border-slate-200 bg-white px-3 text-xs font-black text-slate-600 hover:border-slate-400">' + esc(t(lang, "clearSelection")) + '</a></div>' : ''
    ].join("");
  ...
  function renderKeywordAxisStage(lang, radar, selectedCategory, selectedKeywords) {
    ...
    const body = [
      ...
      rows.map((row) => {
        ...
        return [
          '<tr>',
          '<td class="px-4 py-3 font-black text-slate-950"><a class="hover:text-blue-700" href="' + esc(keywordHref(lang, [row.keyword])) + '">#' + esc(row.keyword || "-") + '</a>' + crossBadge + '</td>',
      ...
      '<a href="' + esc(keywordHref(lang, rows.slice(0, 1).map((row) => row.keyword))) + '" class="inline-flex min-h-[36px] items-center rounded-full border border-slate-200 bg-white px-3 text-xs font-black text-slate-600 hover:border-slate-400">' + esc(t(lang, "openKeywordView")) + '</a>',
      selectedCategory ? '<a href="' + esc(categoryHref(lang, selectedCategory)) + '" class="inline-flex min-h-[36px] items-center rounded-full border border-slate-200 bg-white px-3 text-xs font-black text-slate-600 hover:border-slate-400">' + esc(t(lang, "openCategoryView")) + '</a>' : '',
      '</div>'
    ].join("");
  ...
  function renderKeywordHero(lang, rows) {
    ...
    return [
      ...
      '<a href="' + esc(keywordHref(lang, [])) + '" class="inline-flex w-fit items-center rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-black text-slate-600 hover:border-slate-400">' + esc(t(lang, "clearSelection")) + '</a>',
      '</div>',
  ...
  function renderComparisonHero(lang, rows, loading) {
    ...
    return [
      ...
      '<a href="' + esc(shoppingBasePath(lang)) + '" class="inline-flex w-fit items-center rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-black text-slate-600 hover:border-slate-400">' + esc(t(lang, "clearSelection")) + '</a>',
      '</div>',
      '<div class="grid grid-cols-1 gap-3 md:grid-cols-3">',
