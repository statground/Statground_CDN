(function () {
  ...
  const dict = {
    ko: {
      ...
      keywordStageDesc: "키워드는 여러 카테고리에 동시에 걸칠 수 있습니다. 선택한 카테고리 안의 키워드도 키워드 기준 화면에서 반대쪽 카테고리 분포를 다시 확인할 수 있습니다.",
    ...
    en: {
      ...
      keywordStageDesc: "A keyword can belong to multiple categories. Even inside a selected category, open the keyword view to compare the reverse category distribution.",
  ...
  function categoryAxisTile(lang, item, active, rank) {
    ...
    const button = [
      '<button type="submit" data-market-category="' + esc(name) + '" aria-label="' + esc(t(lang, "submitCategory") + " · " + name) + '" class="group flex min-h-[176px] w-full flex-col rounded-lg border p-3 text-left transition ' + (active ? "border-slate-950 bg-slate-950 text-white shadow-md" : "border-slate-200 bg-white hover:border-blue-300 hover:bg-blue-50") + '">',
      '<div class="flex items-start justify-between gap-3">',
      '<span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full ' + (active ? "bg-white text-slate-950" : "bg-slate-100 text-slate-700 group-hover:bg-white group-hover:text-blue-700") + '">' + categoryIcon(name) + '</span>',
      '<span class="rounded-full ' + (active ? "bg-white/15 text-white" : "bg-slate-100 text-slate-600") + ' px-2 py-1 text-[11px] font-black">#' + esc(rank) + '</span>',
      '</div>',
  ...
  function renderKeywordAxisStage(lang, radar, selectedCategory, selectedKeywords) {
    ...
    const body = [
      ...
      '<thead class="bg-slate-50 text-xs font-black uppercase text-slate-500"><tr><th class="px-4 py-3 text-left">' + esc(t(lang, "query")) + '</th><th class="px-4 py-3 text-right">' + esc(t(lang, "axisProducts")) + '</th><th class="px-4 py-3 text-right">' + esc(t(lang, "axisCategories")) + '</th><th class="px-4 py-3 text-right">' + esc(t(lang, "axisMedian")) + '</th><th class="px-4 py-3 text-right">' + esc(t(lang, "metricDemand")) + '</th><th class="px-4 py-3 text-right">' + esc(t(lang, "metricCompetition")) + '</th><th class="px-4 py-3 text-right">' + esc(t(lang, "metricOpportunity")) + '</th><th class="px-4 py-3 text-left">' + esc(t(lang, "categoryCoverage")) + '</th></tr></thead>',
      ...
      rows.map((row) => {
        ...
        const keywordButton = postSelectionForm(lang, "keyword", row.keyword, '<button type="submit" class="font-black text-slate-950 hover:text-blue-700">#' + esc(row.keyword || "-") + '</button>', "inline");
        return [
          '<tr data-keyword-row data-filter-text="' + esc([row.keyword, (row.categories || []).join(" "), krw(row.median_price_krw || 0)].join(" ")) + '" data-cross-category="' + (Number(row.category_count || 0) > 1 ? "1" : "0") + '" data-opportunity="' + esc(Number(row.opportunity_score || 0)) + '">',
          '<td class="px-4 py-3">' + keywordButton + crossBadge + '</td>',
          ...
          '<td class="px-4 py-3 text-slate-600">' + categorySummary + '</td>',
          '</tr>'
        ].join("");
      }).join(""),
      '</tbody></table></div>',
  ...
  function renderPriceBandCriteriaStage(lang, radar, selectedCategory, selectedKeywords, mode) {
    ...
    const body = [
      ...
      '<section class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">',
      '<div class="mb-3 flex items-center justify-between gap-3"><h3 class="text-sm font-black text-slate-950">' + esc(t(lang, "absolutePriceBands")) + '</h3><span class="rounded-full bg-slate-100 px-2 py-1 text-[11px] font-black text-slate-600">' + esc(t(lang, "priceBandUse")) + '</span></div>',
      '<div class="grid grid-cols-2 gap-2 md:grid-cols-3">',
      absolute.map((row) => '<div class="rounded-lg border border-slate-200 bg-slate-50 p-3"><div class="text-xs font-black text-slate-950">' + esc(row.text) + '</div><div class="mt-1 text-[11px] font-bold text-slate-500">' + esc(number(counts[row.label] || 0)) + ' ' + esc(t(lang, "products")) + '</div></div>').join(""),
      '</div>',
      '</section>',
      '<section class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">',
      ...
      relative.map((row) => '<div class="rounded-lg border border-slate-200 bg-slate-50 p-3"><div class="text-xs font-black text-slate-950">' + esc(row.title) + '</div><div class="mt-1 text-sm font-black text-blue-700">' + esc(row.text) + '</div></div>').join(""),
      '</div>',
      '</section>',
      '</div>'
    ].join("");
