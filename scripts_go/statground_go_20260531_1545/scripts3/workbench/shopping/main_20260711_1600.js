(function () {
  ...
  const ECHARTS_URL = STATGROUND_CDN + ECHARTS_ASSET;
  ...
  function emptyKeywordSearchRadar(radar, query) {
    const baseSummary = radar && radar.summary ? radar.summary : {};
    return Object.assign({}, radar || {}, {
      __sg_keyword_search_query: String(query || "").trim(),
      products: [],
      deal_candidates: [],
      price_drops: [],
      keywords: [],
  ...
  function productDetailModalHTML(lang, item, detailKey) {
    ...
    return [
      ...
      '</div>',
      url ? '<div class="mt-3 flex justify-center"><img src="' + esc(ADPICK_LOGO_URL) + '" alt="Adpick" loading="lazy" class="h-[125px] w-[125px] object-contain"></div>' : '',
      '</div>',
      '</div>',
      '</section>',
      '</div>'
  ...
  function shouldFetchPriceEvidence() {
    const radar = window.__statgroundLastRadar || {};
    return analysisModeFromURL() !== "keyword" && compareSlugsFromURL().length <= 1 && !activeKeywordSearchQuery(radar);
  }
  ...
  function keywordMartSearchParams(query, category) {
    ...
    const range = keywordSearchPriceRange();
    if (range) {
  ...
  function keywordSearchPriceRange() {
    const range = currentPriceRange();
    if (!range) return null;
    const radar = window.__statgroundLastRadar || window.__statgroundBaseRadar || {};
    const extent = observedPriceExtent(radar);
    if (extent && Number(extent.min || 0) > 0 && Number(extent.max || 0) > 0) {
      const coversMin = Number(range.min || 0) <= Number(extent.min || 0);
      const coversMax = range.max === Infinity || Number(range.max || 0) >= Number(extent.max || 0);
      if (coversMin && coversMax) return null;
    }
    return range;
  }
  ...
  function applyKeywordMartResult(lang, radar, appEl, result, mode, options) {
    const opts = options || {};
    ...
    window.__statgroundPriceEvidenceRequestKey = "";
    if (mode === "keyword") {
      renderLoadedKeywordRadar(lang, merged, appEl);
    } else {
      renderLoadedRadar(lang, merged, appEl, {
        baseRadar: opts.baseRadar || window.__statgroundBaseRadar || radar,
        defaultCategory: opts.defaultCategory || window.__statgroundSelectedMarketCategory || "",
        skipKeywordDeepLink: !!opts.skipKeywordDeepLink
      });
    }
    return true;
  }

  function renderKeywordSearchEmptyView(lang, radar, appEl, mode, query, selectedCategory, options) {
    const opts = options || {};
    const emptyRadar = emptyKeywordSearchRadar(radar, query);
    window.__statgroundKeywordSearchQuery = query || window.__statgroundKeywordSearchQuery || "";
    window.__statgroundPriceEvidenceRequestKey = "";
    if (mode === "keyword") {
      renderLoadedKeywordRadar(lang, emptyRadar, appEl);
    } else {
      renderLoadedRadar(lang, emptyRadar, appEl, {
        baseRadar: opts.baseRadar || window.__statgroundBaseRadar || radar,
        defaultCategory: selectedCategory || "",
        skipKeywordDeepLink: !!opts.skipKeywordDeepLink
      });
  ...
  function maybeApplyKeywordDeepLinkSearch(lang, radar, appEl) {
    ...
    const baseRadar = window.__statgroundBaseRadar || radar;
    fetchKeywordMartResult(lang, query, "")
      .then((result) => {
        if (!applyKeywordMartResult(lang, radar, appEl, result, "category", {
          baseRadar: baseRadar,
          defaultCategory: "",
          skipKeywordDeepLink: true
        })) {
          renderKeywordSearchEmptyView(lang, radar, appEl, "category", query, "", {
            baseRadar: baseRadar,
            skipKeywordDeepLink: true
          });
        }
  ...
  function renderInitialKeywordDeepLinkSearch(lang, radar, appEl, query) {
    if (!query || !appEl) return false;
    window.__statgroundKeywordDeepLinkTried = window.__statgroundKeywordDeepLinkTried || {};
    const key = keywordKey(query);
    if (key) window.__statgroundKeywordDeepLinkTried[key] = true;
    window.__statgroundKeywordSearchQuery = query;
    window.__statgroundPriceEvidenceRequestKey = "";
    window.__statgroundSelectedPriceRange = null;
    window.__statgroundSelectedMarketCategory = "";
  ...
  function renderLoadedRadar(lang, radar, appEl, options) {
    ...
    if (!opts.skipKeywordDeepLink && !isKeywordSearchLoading(radar) && !activeKeywordSearchQuery(radar) && maybeApplyKeywordDeepLinkSearch(lang, radar, appEl)) return;
  ...
  function render(langOverride) {
    ...
    function showApp(payload) {
      ...
      currentRadar = Object.assign({}, currentRadar, radar);
      const deepLinkQuery = keywordDeepLinkQueryFromURL();
      if (deepLinkQuery && !keywordMode && compareSlugs.length <= 1) {
        renderInitialKeywordDeepLinkSearch(lang, currentRadar, appEl, deepLinkQuery);
        return;
      }
      renderLoadedRadar(lang, currentRadar, appEl);
    }
    function showError() {
      appEl.innerHTML = '<div class="rounded-lg border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">' + esc(t(lang, "loadError")) + '</div>';
    }
    ...
    const coreParams = radarCategory ? { category: radarCategory, view: "core" } : { view: "core" };
    const detailParams = radarCategory ? { category: radarCategory, view: "details" } : { view: "details" };
    ...
    function loadRadarDetails() {
      return fetchJSONWithRetry(radarDetailsURL, 2).then(function (res) {
        const patch = res && res.ok && res.json ? res.json.radar_patch : null;
        if (!patch) return;
        currentRadar = Object.assign({}, currentRadar, patch);
        if (!keywordDeepLinkQueryFromURL() && compareSlugs.length <= 1) {
          renderLoadedRadar(lang, currentRadar, appEl);
        }
      }).catch(function () {});
    }
    ...
    if (early && early.url === radarURL && early.promise) {
      ...
      loadRadar().then(showCoreThenDetails).catch(showError);
    }
  }
