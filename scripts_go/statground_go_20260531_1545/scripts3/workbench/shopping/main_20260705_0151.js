(function () {
  ...
  const dict = {
    ko: {
      ...
      closeDetails: "닫기",
      affiliatePartner: "Adpick 제휴 링크",
      affiliateCaption: "Adpick 개인/BIZ API를 통해 판매처 현재가 링크로 연결됩니다.",
      saveWatch: "알림 후보 저장",
      savedWatch: "저장됨",
      removeWatch: "삭제",
      routerNotice: "",
    ...
    en: {
      ...
      closeDetails: "Close",
      affiliatePartner: "Adpick affiliate link",
      affiliateCaption: "Outbound seller links are routed through Adpick personal/BIZ APIs before opening the current marketplace price.",
      ...
      prevPage: "Previous",
      nextPage: "Next",
      chartRailHint: "Scroll horizontally through charts",
  ...
  function chartBox(id, title) {
    ...
    return [
      '<section class="min-w-[min(88vw,620px)] snap-start rounded-lg border border-slate-200 bg-white p-3 shadow-sm sm:min-w-[560px] xl:min-w-[620px]">',
  ...
  function chartRailHTML(lang, boxes) {
    ...
    return [
      '<section class="relative" data-chart-rail>',
      '<div class="mb-2 flex items-center justify-between gap-3">',
      '<span class="text-xs font-bold text-slate-500">' + esc(t(lang, "chartRailHint")) + '</span>',
      '<div class="inline-flex items-center gap-2">',
      '<button type="button" data-chart-rail-prev class="flex h-9 w-9 items-center justify-center rounded-full bg-slate-900 text-lg font-black leading-none text-white shadow-sm hover:bg-slate-700" aria-label="' + esc(t(lang, "prevPage")) + '">&lt;</button>',
      '<button type="button" data-chart-rail-next class="flex h-9 w-9 items-center justify-center rounded-full bg-slate-900 text-lg font-black leading-none text-white shadow-sm hover:bg-slate-700" aria-label="' + esc(t(lang, "nextPage")) + '">&gt;</button>',
      '</div>',
      '</div>',
      '<div data-chart-rail-track class="flex snap-x gap-3 overflow-x-auto scroll-smooth pb-2">',
      rows.join(""),
      '</div>',
      '</section>'
    ].join("");
  ...
  function renderAnalysisPath(lang, mode, selectedCategory, selectedKeywords) {
    ...
    return [
      '<section class="mb-5 rounded-lg border border-slate-200 bg-white px-3 py-2 shadow-sm">',
      ...
      steps.map((step, index) => [
        '<span class="inline-flex min-h-[28px] items-center gap-1.5 rounded-full border px-2.5 py-1 ' + (step.active ? "border-slate-900 bg-slate-950 text-white" : "border-slate-200 bg-slate-50 text-slate-500") + '">',
        '<span class="text-[11px] font-black">' + esc(index + 1) + '</span>',
  ...
  function renderCategoryAxisStage(lang, radar, selectedCategory, selectedSlugs) {
    ...
    const body = [
      '<p class="mb-3 text-xs font-bold text-slate-500">' + esc(t(lang, "categoryIconOnlyHint")) + '</p>',
      '<div class="mb-3 grid grid-cols-1 gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">',
  ...
  function renderComparisonProducts(lang, rows) {
    ...
    return [
      ...
      visible.map((row) => [
        ...
        '<div class="mt-3 ' + productGridClass() + '">',
  ...
  function productDetailModalHTML(lang, item, detailKey) {
    ...
    return [
      ...
      '<div class="flex items-center gap-2">',
      '<img src="' + esc(ADPICK_LOGO_URL) + '" alt="Adpick" loading="lazy" class="h-6 w-auto object-contain">',
      '<span class="text-xs font-black text-slate-700">' + esc(t(lang, "affiliatePartner")) + '</span>',
      '</div>',
      '<p class="mt-2 text-xs font-bold leading-5 text-slate-500">' + esc(t(lang, "affiliateCaption")) + '</p>',
      '</div>',
      ...
      url ? '<a href="' + esc(url) + '" target="_blank" rel="noopener noreferrer" class="inline-flex min-h-[40px] items-center justify-center rounded-lg bg-slate-900 px-3 text-sm font-black text-white hover:bg-slate-700">' + esc(t(lang, "source")) + '</a>' : '',
      '</div>',
      '</div>',
      '</div>',
      '</section>',
  ...
  function openProductDetailModal(lang, key) {
  ...
  function renderObservedProducts(lang, radar, selectedCategory) {
    ...
    return '<section>' + productListHeaderHTML(lang, selectedCategory + " · " + t(lang, "categoryObservedProducts"), items.length) + '<div class="' + productGridClass() + '">' + items.map((item) => productCard(lang, item)).join("") + '</div></section>';
  }
  ...
  function setupChartRails(appEl) {
    ...
    scope.querySelectorAll("[data-chart-rail]").forEach((rail) => {
      ...
      function scrollByPage(direction) {
        track.scrollBy({ left: direction * Math.max(240, Math.round(track.clientWidth * 0.9)), behavior: "smooth" });
      }
