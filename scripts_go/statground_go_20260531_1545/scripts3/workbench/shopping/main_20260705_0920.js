(function () {
  ...
  function productDetailModalHTML(lang, item, detailKey) {
    ...
    return [
      ...
      url ? '<div class="flex min-w-0 flex-col items-center gap-2"><a href="' + esc(url) + '" target="_blank" rel="noopener noreferrer" class="inline-flex min-h-[40px] w-full items-center justify-center rounded-lg bg-slate-900 px-3 text-center text-sm font-black text-white hover:bg-slate-700">' + esc(t(lang, "source")) + '</a><img src="' + esc(ADPICK_LOGO_URL) + '" alt="Adpick" loading="lazy" class="h-7 w-auto max-w-[150px] object-contain"></div>' : '',
      '</div>',
      '</div>',
      '</div>',
      '</section>',
