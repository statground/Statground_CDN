(function () {
  ...
  function categoryTabHTML(lang, value, title, sub, active) {
    return [
      '<button type="button" data-top-category="' + esc(value || "") + '" aria-pressed="' + (active ? "true" : "false") + '" class="shrink-0 rounded-lg border px-4 py-3 text-left transition ' + (active ? "border-slate-950 bg-slate-950 text-white shadow-sm" : "border-slate-200 bg-white text-slate-700 hover:border-blue-300 hover:bg-blue-50") + '">',
      '<span class="block text-sm font-black leading-5">' + esc(title || "-") + '</span>',
      sub ? '<span class="mt-1 block text-xs font-bold leading-4 ' + (active ? "text-slate-200" : "text-slate-500") + '">' + esc(sub) + '</span>' : '',
      '</button>'
    ].join("");
  }
  ...
  function renderInsightCharts(lang, radar, finder) {
    ...
    const visibleCategories = selected ? radarCategories.filter((row) => row && row.source_category === selected) : radarCategories;
    ...
    const visibleSellers = selected ? radarCategories.filter((row) => row && row.source_category === selected) : radarCategories;
