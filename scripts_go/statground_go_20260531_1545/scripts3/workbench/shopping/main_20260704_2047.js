(function () {
  ...
  const PRICE_RANGE_PRESETS = [
    ...
    { key: "range:0-5000", min: 0, max: 5000 },
    { key: "range:5001-10000", min: 5001, max: 10000 },
    { key: "range:10001-30000", min: 10001, max: 30000 },
    { key: "range:30001-50000", min: 30001, max: 50000 },
    { key: "range:50001-100000", min: 50001, max: 100000 },
    { key: "range:100001-inf", min: 100001, max: Infinity }
  ];
  const PRICE_RANGE_SLIDER_BOUNDARIES = [0, 5000, 10000, 30000, 50000, 100000, Infinity];
  ...
  const dict = {
    ko: {
      ...
      priceRangeDragHint: "핸들을 드래그해 범위를 조정",
    ...
    en: {
      ...
      priceRangeDragHint: "Drag the handles to adjust the range",
  ...
  function priceRangeSliderMaxIndex() {
    return PRICE_RANGE_SLIDER_BOUNDARIES.length - 1;
  }

  function clampPriceRangeSliderIndex(value) {
    const max = priceRangeSliderMaxIndex();
    const n = Math.round(Number(value || 0));
    if (!Number.isFinite(n)) return 0;
    return Math.max(0, Math.min(max, n));
  }

  function priceRangeFromSliderIndexes(minIndex, maxIndex) {
    let lowIndex = clampPriceRangeSliderIndex(minIndex);
    let highIndex = clampPriceRangeSliderIndex(maxIndex);
    const last = priceRangeSliderMaxIndex();
    if (highIndex <= lowIndex) {
      if (lowIndex >= last) lowIndex = last - 1;
      highIndex = lowIndex + 1;
    }
    if (lowIndex <= 0 && highIndex >= last) return null;
    const lowBoundary = PRICE_RANGE_SLIDER_BOUNDARIES[lowIndex];
    const highBoundary = PRICE_RANGE_SLIDER_BOUNDARIES[highIndex];
    const min = lowIndex <= 0 ? 0 : Number(lowBoundary || 0) + 1;
    const max = highBoundary === Infinity ? Infinity : Number(highBoundary || 0);
    return normalizePriceRange({ key: priceRangeKey(min, max), min: min, max: max });
  }

  function priceRangeSliderIndexes(rawRange) {
    const range = normalizePriceRange(rawRange);
    const last = priceRangeSliderMaxIndex();
    if (!range) return { minIndex: 0, maxIndex: last };
    let minIndex = 0;
    let maxIndex = last;
    for (let i = 0; i < PRICE_RANGE_SLIDER_BOUNDARIES.length; i += 1) {
      const boundary = PRICE_RANGE_SLIDER_BOUNDARIES[i];
      if (boundary !== Infinity && Number(boundary || 0) + 1 <= Number(range.min || 0)) minIndex = i;
      if (range.max !== Infinity && boundary !== Infinity && Number(boundary || 0) >= Number(range.max || 0)) {
        maxIndex = i;
        break;
      }
    }
  ...
  function priceRangeSliderPercent(index) {
    const last = priceRangeSliderMaxIndex();
    if (!last) return 0;
    return Math.round((1000 * clampPriceRangeSliderIndex(index)) / last) / 10;
  }

  function priceRangeBoundaryLabel(index) {
    const boundary = PRICE_RANGE_SLIDER_BOUNDARIES[clampPriceRangeSliderIndex(index)];
    if (boundary === Infinity) return krw(PRICE_RANGE_SLIDER_BOUNDARIES[priceRangeSliderMaxIndex() - 1]) + "+";
    if (boundary === 0) return krw(0);
    return krw(boundary);
  }
  ...
  function renderPriceBandCriteriaStage(lang, radar, selectedCategory, selectedKeywords, mode) {
    ...
    const slider = priceRangeSliderIndexes(activeRange);
    const selectedLabel = priceRangeLabel(lang, activeRange);
    const body = [
      '<div class="grid grid-cols-1 gap-4 xl:grid-cols-[1fr_0.8fr]">',
      '<section class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm" id="sg-shopping-price-range-slider">',
      ...
      '<div data-price-range-slider-track class="absolute top-6 h-2 rounded-full bg-blue-600" style="left:' + esc(priceRangeSliderPercent(slider.minIndex)) + '%;right:' + esc(100 - priceRangeSliderPercent(slider.maxIndex)) + '%"></div>',
      '<input type="range" min="0" max="' + esc(priceRangeSliderMaxIndex()) + '" step="1" value="' + esc(slider.minIndex) + '" data-price-range-slider-min aria-label="' + esc(t(lang, "priceRangeMin")) + '" class="sg-price-range-input z-20">',
      '<input type="range" min="0" max="' + esc(priceRangeSliderMaxIndex()) + '" step="1" value="' + esc(slider.maxIndex) + '" data-price-range-slider-max aria-label="' + esc(t(lang, "priceRangeMax")) + '" class="sg-price-range-input z-30">',
      '</div>',
      '<div class="flex justify-between gap-2 text-[11px] font-black text-slate-500">',
      PRICE_RANGE_SLIDER_BOUNDARIES.map((_, index) => '<span>' + esc(priceRangeBoundaryLabel(index)) + '</span>').join(""),
      '</div>',
      '</div>',
      '<div class="mt-4"><div class="mb-2 text-xs font-black uppercase text-slate-500">' + esc(t(lang, "priceRangeQuick")) + '</div><div class="flex flex-wrap gap-2">',
      ...
      '</section>',
      '<section class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">',
      '<div class="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><h3 class="text-sm font-black text-slate-950">' + esc(t(lang, "relativePriceBands")) + '</h3><p class="mt-1 text-xs font-bold text-slate-500">' + esc(t(lang, "priceRangeManual")) + '</p></div><span class="inline-flex w-fit rounded-full bg-slate-100 px-2 py-1 text-[11px] font-black text-slate-600">' + esc(t(lang, "priceBandScope")) + ' · ' + esc(scope) + '</span></div>',
      ...
      '<label class="block"><span class="text-xs font-black text-slate-500">' + esc(t(lang, "priceRangeMax")) + '</span><input type="number" min="0" step="100" inputmode="numeric" data-price-range-max class="mt-1 h-11 w-full rounded-lg border border-slate-200 px-3 text-sm font-black text-slate-800 outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100" placeholder="50000"></label>',
      '</div>',
      '<div class="mt-3 flex flex-wrap gap-2">',
      '<button type="button" data-price-range-custom-apply class="inline-flex min-h-[40px] items-center rounded-lg bg-slate-900 px-4 text-sm font-black text-white hover:bg-slate-700">' + esc(t(lang, "priceRangeApply")) + '</button>',
      '<button type="button" data-price-range-clear class="inline-flex min-h-[40px] items-center rounded-lg border border-slate-200 bg-white px-4 text-sm font-black text-slate-700 hover:border-slate-400">' + esc(t(lang, "priceRangeClear")) + '</button>',
      '</div>',
      '</section>',
      '</div>'
    ].join("");
  ...
  function bindPriceBandCriteria(lang, appEl) {
    ...
    function sliderRangeFromInputs() {
      ...
      return priceRangeFromSliderIndexes(sliderMin.value, sliderMax.value);
    }
    function setInputsForRange(range) {
      const current = normalizePriceRange(range);
      if (minInput) minInput.value = current ? String(current.min || 0) : "";
      if (maxInput) maxInput.value = current && current.max !== Infinity ? String(current.max || 0) : "";
    }
    function syncSliderPreview(range) {
      const normalized = normalizePriceRange(range);
      const indexes = priceRangeSliderIndexes(normalized);
      if (sliderMin) sliderMin.value = String(indexes.minIndex);
      if (sliderMax) sliderMax.value = String(indexes.maxIndex);
      if (sliderTrack) {
        sliderTrack.style.left = priceRangeSliderPercent(indexes.minIndex) + "%";
        sliderTrack.style.right = (100 - priceRangeSliderPercent(indexes.maxIndex)) + "%";
      }
    ...
    function normalizeSliderHandles(changed) {
      ...
      let low = clampPriceRangeSliderIndex(sliderMin.value);
      let high = clampPriceRangeSliderIndex(sliderMax.value);
      const last = priceRangeSliderMaxIndex();
      if (high <= low) {
    ...
    const applyButton = appEl.querySelector("[data-price-range-custom-apply]");
    if (applyButton) {
      applyButton.addEventListener("click", function () {
        ...
        if (!Number.isFinite(min) || min < 0 || (max !== Infinity && (!Number.isFinite(max) || max < min))) {
