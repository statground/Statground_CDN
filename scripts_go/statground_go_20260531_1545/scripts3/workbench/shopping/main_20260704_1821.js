(function () {
  ...
  const dict = {
    ko: {
      ...
      desc: "Gmarket과 Kurly에서 관측한 카테고리, 키워드, 가격대, 기회 신호를 원본 재게시 없이 집계·파생 지표로 분석합니다.",
      notice: "이 화면은 상품 상세를 복제하지 않고 플랫폼별 관측 상품과 카테고리·키워드·가격대의 집계 지표를 보여줍니다. 표시 가격과 반응 지표는 수집 시점 관측값이며 실제 구매·판매 판단 전에는 외부몰과 제휴 정책을 확인해야 합니다.",
      budget: "예산",
      category: "카테고리",
      query: "찾는 것",
      ...
      categoryDeckDesc: "카테고리를 선택하면 해당 범위의 가격대, 키워드, 기회 점수, 관측 상품만 집중해서 보여줍니다.",
      ...
      categoryFirstDesc: "표준 10개 카테고리에서 분석 렌즈를 고른 뒤 시장, 가격대, 키워드, 상품 근거로 내려갑니다.",
      ...
      priceBandStageTitle: "2. 가격대 기준 잡기",
      priceBandStageDesc: "절대 가격 버킷과 선택 렌즈의 P25·P50·P75 상대 기준을 함께 보며 가격 해석 기준을 먼저 고정합니다.",
      absolutePriceBands: "절대 가격대",
      relativePriceBands: "상대 분위 기준",
      priceBandScope: "적용 렌즈",
      priceBandUse: "차트와 후보 평가는 이 가격대 기준으로 묶어 해석합니다.",
      priceBandSelected: "선택 기준",
      priceBandUnselected: "가격대 또는 분위 기준을 선택해 주세요.",
      selectPriceBand: "가격대 선택",
      relativeLow: "저가권",
    ...
    en: {
      back: "Workbench",
      title: "Shopping Price Insight",
      desc: "Analyze category, keyword, price-band, and opportunity signals observed from Gmarket and Kurly without republishing marketplace content.",
      notice: "This view does not replicate product detail pages. It shows provider-aware observed products plus aggregate metrics by category, keyword, and price band. Prices and reaction signals are observations at collection time; verify the external mall and affiliate policy before buying or selling decisions.",
      budget: "Budget",
      category: "Category",
      query: "Need",
      ...
      categoryDeckDesc: "Select a category to focus price bands, keywords, opportunity scores, and observed products within that category.",
      ...
      categoryFirstDesc: "Start from the 10 standard category lenses, then move down into market, price bands, keywords, and evidence.",
      ...
      priceBandStageTitle: "2. Set price-band criteria",
      priceBandStageDesc: "Use absolute price buckets together with the selected lens P25, P50, and P75 thresholds before interpreting price signals.",
      absolutePriceBands: "Absolute bands",
      relativePriceBands: "Relative quantiles",
      priceBandScope: "Applied lens",
      priceBandUse: "Charts and candidates are grouped against these price-band criteria.",
      priceBandSelected: "Selected criteria",
      priceBandUnselected: "Select an absolute or relative price criterion.",
      selectPriceBand: "Select price band",
  ...
  function priceBandButtonHTML(lang, key, title, detail, meta) {
    const active = window.__statgroundSelectedPriceBandKey === key;
    const label = [title, detail].filter(Boolean).join(" · ");
    return [
      '<button type="button" data-price-band-key="' + esc(key) + '" data-price-band-label="' + esc(label) + '" aria-label="' + esc(t(lang, "selectPriceBand") + " · " + label) + '" aria-pressed="' + (active ? "true" : "false") + '" class="' + priceBandButtonClass(active) + '">',
      '<div data-price-band-title class="text-xs font-black ' + (active ? "text-white" : "text-slate-950") + '">' + esc(title) + '</div>',
      '<div data-price-band-value class="mt-1 text-sm font-black ' + (active ? "text-white" : "text-blue-700") + '">' + esc(detail) + '</div>',
      meta ? '<div data-price-band-meta class="mt-1 text-[11px] font-bold ' + (active ? "text-white/75" : "text-slate-500") + '">' + esc(meta) + '</div>' : '',
      '</button>'
    ].join("");
  }

  function priceBandButtonClass(active) {
    return "rounded-lg border p-3 text-left transition focus:outline-none focus:ring-2 focus:ring-blue-300 " + (active ? "border-slate-950 bg-slate-950 text-white shadow-md" : "border-slate-200 bg-slate-50 hover:border-blue-300 hover:bg-blue-50");
  }

  function renderPriceBandCriteriaStage(lang, radar, selectedCategory, selectedKeywords, mode) {
    const reference = selectedPriceReferenceRow(radar, selectedCategory, selectedKeywords, mode);
    const p25v = Number(reference && reference.p25_price_krw || 0);
    const p50v = Number(reference && (reference.median_price_krw || reference.p50_price_krw) || 0);
    const p75v = Number(reference && reference.p75_price_krw || 0);
    const counts = {};
    priceBands(radar, []).forEach((row) => {
      counts[row.label] = Number(row.count || 0);
    });
    const absolute = [
      { label: "<=5k", text: "~₩5,000" },
      { label: "5k-10k", text: "₩5,001~₩10,000" },
      { label: "10k-30k", text: "₩10,001~₩30,000" },
      { label: "30k-50k", text: "₩30,001~₩50,000" },
      { label: "50k-100k", text: "₩50,001~₩100,000" },
      { label: "100k+", text: "₩100,001~" }
    ];
    const relative = [
      { title: t(lang, "relativeLow"), text: "≤ " + (p25v ? krw(p25v) : "P25") },
      { title: t(lang, "relativeCenterLow"), text: (p25v ? krw(p25v) : "P25") + " ~ " + (p50v ? krw(p50v) : "P50") },
      { title: t(lang, "relativeCenterHigh"), text: (p50v ? krw(p50v) : "P50") + " ~ " + (p75v ? krw(p75v) : "P75") },
      { title: t(lang, "relativePremium"), text: "> " + (p75v ? krw(p75v) : "P75") }
    ];
    const scope = selectedCategory || ((selectedKeywords || []).length ? selectedKeywords.join(", ") : t(lang, "allMarket"));
    const selectedLabel = window.__statgroundSelectedPriceBandLabel || t(lang, "priceBandUnselected");
    const body = [
      '<div class="grid grid-cols-1 gap-4 xl:grid-cols-[1.1fr_0.9fr]">',
      '<section class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">',
      '<div class="mb-3 flex items-center justify-between gap-3"><h3 class="text-sm font-black text-slate-950">' + esc(t(lang, "absolutePriceBands")) + '</h3><span id="sg-shopping-price-band-status" class="rounded-full bg-slate-100 px-2 py-1 text-[11px] font-black text-slate-600">' + esc(t(lang, "priceBandSelected")) + ' · ' + esc(selectedLabel) + '</span></div>',
      '<div class="grid grid-cols-2 gap-2 md:grid-cols-3">',
      absolute.map((row) => priceBandButtonHTML(lang, "absolute:" + row.label, row.label, row.text, number(counts[row.label] || 0) + " " + t(lang, "products"))).join(""),
      '</div>',
      '</section>',
      '<section class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">',
      '<div class="mb-3 flex items-center justify-between gap-3"><h3 class="text-sm font-black text-slate-950">' + esc(t(lang, "relativePriceBands")) + '</h3><span class="rounded-full bg-slate-100 px-2 py-1 text-[11px] font-black text-slate-600">' + esc(t(lang, "priceBandScope")) + ' · ' + esc(scope) + '</span></div>',
      '<div class="grid grid-cols-1 gap-2 sm:grid-cols-2">',
      relative.map((row) => priceBandButtonHTML(lang, "relative:" + row.title, row.title, row.text, scope)).join(""),
      '</div>',
      '</section>',
      '</div>'
    ].join("");
  ...
  function scopedCategoryKeywordRows(radar, selectedCategory) {
    const rows = ((radar && radar.category_keywords) || []).filter((item) => item && item.keyword);
  ...
  function renderKeywordDiscovery(lang, radar, selectedCategory) {
    ...
    if (selectedCategory && !rows.length) rows = ((radar && radar.keywords) || []);
    rows = rows
      .filter((item) => item && item.keyword)
      .slice()
      .sort((a, b) => {
  ...
  function renderKeywordDashboard(lang, radar) {
    ...
    return [
      ...
      renderMarketOverviewStage(lang, radar, ""),
      renderKeywordAxisStage(lang, radar, "", selected),
      renderSegmentStage(lang, radar, "", selected, "keyword"),
      renderEvidenceStage(lang, radar, "", selected, "keyword")
    ].join("");
  }
  ...
  function renderComparisonDashboard(lang, baseRadar, rows, selectedSlugs, loading) {
    return [
      ...
      renderMarketOverviewStage(lang, baseRadar, ""),
  ...
  function renderResearchDashboard(lang, radar) {
    ...
    return [
      ...
      renderMarketOverviewStage(lang, radar, selected),
      renderKeywordAxisStage(lang, radar, selected, []),
      renderSegmentStage(lang, radar, selected, [], "category"),
      renderEvidenceStage(lang, radar, selected, [], "category")
    ].join("");
  }
  ...
  function bindPriceBandCriteria(lang, appEl) {
    ...
    function sync() {
      buttons.forEach((btn) => {
        const active = btn.getAttribute("data-price-band-key") === window.__statgroundSelectedPriceBandKey;
        btn.setAttribute("aria-pressed", active ? "true" : "false");
        btn.className = priceBandButtonClass(active);
        const title = btn.querySelector("[data-price-band-title]");
        const value = btn.querySelector("[data-price-band-value]");
        const meta = btn.querySelector("[data-price-band-meta]");
        if (title) title.className = "text-xs font-black " + (active ? "text-white" : "text-slate-950");
        if (value) value.className = "mt-1 text-sm font-black " + (active ? "text-white" : "text-blue-700");
        if (meta) meta.className = "mt-1 text-[11px] font-bold " + (active ? "text-white/75" : "text-slate-500");
      });
      if (status) {
        status.textContent = t(lang, "priceBandSelected") + " · " + (window.__statgroundSelectedPriceBandLabel || t(lang, "priceBandUnselected"));
      }
    }
    buttons.forEach((btn) => {
      btn.addEventListener("click", function () {
        window.__statgroundSelectedPriceBandKey = btn.getAttribute("data-price-band-key") || "";
        window.__statgroundSelectedPriceBandLabel = btn.getAttribute("data-price-band-label") || "";
        sync();
      });
    });
    sync();
  ...
  function loadComparisonDashboard(lang, radar, appEl, slugs) {
    ...
    Promise.all(requests).then((radars) => {
      const rows = comparisonRows(radar, selected.map((slug, index) => ({ slug: slug, radar: radars[index] })));
  ...
  function renderLoadedKeywordRadar(lang, radar, appEl) {
    ...
    bindWatchButtons(lang, rows.reduce((items, row) => items.concat(keywordProducts(radar, row, selectedKeywords.length ? 60 : 24)), []));
  }
  ...
  function renderLoadedRadar(lang, radar, appEl) {
    ...
    appEl.innerHTML = renderResearchDashboard(lang, radar);
    renderInsightCharts(lang, radar, null);
    paintCategoryKeywordLensCharts(lang, categoryKeywordLensRows(radar, window.__statgroundSelectedMarketCategory || ""));
    bindResearchDashboard(lang, radar, appEl);
    bindPriceBandCriteria(lang, appEl);
    setupShoppingFilters(appEl);
  }
