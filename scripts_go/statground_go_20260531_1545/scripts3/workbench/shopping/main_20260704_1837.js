(function () {
  ...
  function priceBandButtonHTML(lang, key, title, detail, meta, min, max) {
    const active = activePriceRangeKey() === key;
    const label = [title, detail].filter(Boolean).join(" · ");
    return [
      '<button type="button" data-price-band-key="' + esc(key) + '" data-price-band-label="' + esc(title) + '" data-price-min="' + esc(min || 0) + '" data-price-max="' + esc(priceRangeDataMax(max)) + '" aria-label="' + esc(t(lang, "selectPriceBand") + " · " + label) + '" aria-pressed="' + (active ? "true" : "false") + '" class="' + priceBandButtonClass(active) + '">',
  ...
  function renderPriceBandCriteriaStage(lang, radar, selectedCategory, selectedKeywords, mode) {
    ...
    const body = [
      ...
      '<section class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">',
      '<div class="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><h3 class="text-sm font-black text-slate-950">' + esc(t(lang, "absolutePriceBands")) + '</h3><span id="sg-shopping-price-band-status" class="inline-flex w-fit rounded-full bg-slate-100 px-2 py-1 text-[11px] font-black text-slate-600">' + esc(t(lang, "priceBandSelected")) + ' · ' + esc(selectedLabel) + '</span></div>',
      ...
      PRICE_RANGE_PRESETS.map((preset) => {
        ...
        return priceBandButtonHTML(lang, preset.key, label, number(count) + " " + t(lang, "products"), preset.key === "all" ? t(lang, "priceRangeClear") : scope, preset.min, preset.max);
      }).join(""),
      '</div>',
      '<p class="mt-3 text-xs font-bold leading-5 text-slate-500">' + esc(t(lang, "priceBandUse")) + '</p>',
      '</section>',
      '<section class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">',
      '<div class="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><h3 class="text-sm font-black text-slate-950">' + esc(t(lang, "relativePriceBands")) + '</h3><span class="inline-flex w-fit rounded-full bg-slate-100 px-2 py-1 text-[11px] font-black text-slate-600">' + esc(t(lang, "priceBandScope")) + ' · ' + esc(scope) + '</span></div>',
  ...
  function bindPriceBandCriteria(lang, appEl) {
    ...
    function sync() {
      ...
      if (status) {
        status.textContent = t(lang, "priceBandSelected") + " · " + priceRangeLabel(lang, currentPriceRange());
      }
      const current = currentPriceRange();
