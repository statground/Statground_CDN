...
// =====================================
const STATGROUND_BOOK_DETAIL_CDN_ROOT = (() => {
  const scriptURL = typeof document !== "undefined" && document.currentScript && document.currentScript.src
    ? document.currentScript.src
    : "";
  const marker = "/scripts_go/statground_go_20260531_1545/";
  const index = scriptURL.indexOf(marker);
  return index > 0 ? scriptURL.slice(0, index + 1) : "/";
})();

function statgroundBookDetailAsset(path) {
  return STATGROUND_BOOK_DETAIL_CDN_ROOT + String(path || "").replace(/^\/+/, "");
}
...
async function set_main() {
  ...
  (function sgwbEnsureBookDetailDarkFallback(){
    ...
    style.textContent = `
      ...
      /* Adpick promo image must stay readable (force white card) */
      html.dark #div_main .sg-adpick-card,
      html.dark #div_main .sg-adpick-card.bg-white,
      html.dark #div_main .sg-adpick-card.bg-white.rounded-2xl{
  ...
  function render(book, uiLang) {
    ...
    // Marketplace links. Kyobo/YES24 go through the server so Adpick BIZ/personal
    // API calls, cache lookup, fallback, and click logging stay backend-owned.
    const getMarketplaceLogo = (name) => {
      const raw = (name || "");
      const n = raw.toLowerCase();
      if (raw.includes("교보") || n.includes("kyobo")) return statgroundBookDetailAsset("images/data/book/kyobobook.png");
      if (n.includes("yes24") || raw.includes("예스24") || raw.includes("예스")) return statgroundBookDetailAsset("images/data/book/yes24.png");
      if (raw.includes("알라딘") || n.includes("aladin")) return statgroundBookDetailAsset("images/data/book/aladin.png");
      return "";
    };

    const isbnDigits = (isbnRaw || isbn || "").replace(/[^0-9Xx]/g, "");
    const qIsbn = encodeURIComponent(isbnDigits || (isbn || ""));
    const pathIsbn = encodeURIComponent(isbnDigits || (isbn || ""));

    const marketplaces = [
      { name: "교보문고", url: `/${routeLang}/workbench/book/affiliate/${pathIsbn}/kyobo/`, affiliate: true },
      { name: "YES24", url: `/${routeLang}/workbench/book/affiliate/${pathIsbn}/yes24/`, affiliate: true },
    ...
    const marketplaceCardsHtml = marketplacesView
      ...
      .map(m => {
        ...
        const rel = m.affiliate ? "nofollow sponsored noopener noreferrer" : "noopener";
        ...
        return `
          <a href="${safeUrl}" target="_blank" rel="${rel}"
            ...
            <div class="h-10 w-10 rounded-lg bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 overflow-hidden flex items-center justify-center">
              ${logo ? `<img src="${logo}" alt="${safeName}" width="40" height="40" class="h-full w-full object-contain" loading="lazy" decoding="async" />` : ``}
            </div>
    ...
    const adpickCardHtml = `
      <div class="sg-adpick-card rounded-xl border border-slate-200 bg-white p-3 flex min-h-[148px] flex-col items-center justify-center">
        <img src="${statgroundBookDetailAsset("images/common/affiliates/adpick.png")}"
             alt="이글의 홍보링크로 구매하시면 수수료를 제공받습니다." width="125" height="125"
             class="w-[125px] h-[125px] object-contain" loading="lazy" decoding="async" />
      </div>
    `;
        ...
        <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm">
          ...
          <div class="grid grid-cols-12 gap-6">
            <div class="col-span-12 md:col-span-4">
              <div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950 overflow-hidden" style="aspect-ratio: 2 / 3;">
                ${coverUrl ? `<img src="${coverUrl}" alt="${title}" class="w-full h-full object-cover" loading="eager" decoding="async" onerror="this.style.display='none';" />` : ``}
              </div>
            </div>
...
// ---------------- explicit language change handling ----------------
  ...
  window.addEventListener("sg_lang_changed", (e) => {
    ...
    if (newLang && syncLangToURL(newLang)) return;
