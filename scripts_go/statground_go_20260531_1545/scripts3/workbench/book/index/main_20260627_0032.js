...
(function () {
  ...
  // Tailwind CDN may fail to generate `dark:*` utilities for classes defined inside external JS.
  // When that happens, the button keeps `text-white` in dark mode and becomes invisible on `bg-white`.
  // This fallback forces correct contrast only when `html.dark` is active.
  (function ensure_dark_fallback_css(){
    const STYLE_ID = "sgwb-dark-fallback-css";
    if (document.getElementById(STYLE_ID)) return;
    const style = document.createElement("style");
    style.id = STYLE_ID;
    style.textContent = `
      /* ===== Statground Workbench Book : Dark fallback (main.js only) ===== */
      /* Activate ONLY when html.dark is present */
      html.dark{ color-scheme: dark; }
      html.dark body{ background: radial-gradient(1200px 600px at 20% -10%, rgba(99,102,241,0.18), transparent 60%), radial-gradient(900px 500px at 90% 0%, rgba(56,189,248,0.12), transparent 55%), #020617 !important; color:#e2e8f0 !important; }

      /* Header / Footer (Tailwind dark:* may not be generated on this page) */
      html.dark header,
      html.dark nav,
      html.dark #div_header,
      html.dark #header,
      html.dark .sg-header{ background: rgba(2,6,23,0.88) !important; color:#e2e8f0 !important; border-color: rgba(148,163,184,0.18) !important; }
      html.dark footer,
      html.dark #div_footer,
      html.dark .sg-footer{ background: rgba(2,6,23,0.88) !important; color: rgba(226,232,240,0.8) !important; border-color: rgba(148,163,184,0.18) !important; }

      /* Main container */
      html.dark #div_main{ background: transparent !important; }
      html.dark #div_main .bg-white{ background: rgba(15,23,42,0.72) !important; }
      html.dark #div_main .border-slate-200{ border-color: rgba(148,163,184,0.18) !important; }
      html.dark #div_main .text-slate-900{ color:#e2e8f0 !important; }
      html.dark #div_main .text-slate-600{ color: rgba(226,232,240,0.75) !important; }
      html.dark #div_main .text-slate-500{ color: rgba(226,232,240,0.6) !important; }

      /* Inputs */
      html.dark #div_main input{ background: rgba(15,23,42,0.65) !important; color:#e2e8f0 !important; border-color: rgba(148,163,184,0.25) !important; }
      html.dark #div_main input::placeholder{ color: rgba(226,232,240,0.65) !important; }

      /* Search button: ensure text is visible */
      html.dark #div_main .sgwb-search-btn{ background:#ffffff !important; color:#0f172a !important; border-color: rgba(148,163,184,0.35) !important; }
      html.dark #div_main .sgwb-search-btn *{ color:#0f172a !important; }

      /* Also fix generic buttons that might become white-on-white */
      html.dark #div_main button.bg-white,
      html.dark #div_main button[class*='bg-white']{ color:#0f172a !important; }

      /* Card hover: if Tailwind dark:hover:* utilities are missing, light hover (bg-slate-50) can turn cards too white.
         Override ONLY in dark mode and ONLY inside this page. */
      html.dark #div_main .hover\\:bg-slate-50:hover{ background: rgba(255,255,255,0.06) !important; }
      html.dark #div_main .dark\\:hover\\:bg-slate-950:hover{ background: rgba(255,255,255,0.06) !important; }
      html.dark #div_main .hover\\:bg-slate-50:hover,
      html.dark #div_main .dark\\:hover\\:bg-slate-950:hover{ border-color: rgba(148,163,184,0.28) !important; }
    `;
...
const readEarlyRecent = async (url) => {
        const early = window.__statgroundBookRecentEarlyFetch;
        if (!early || early.url !== url || !early.promise || typeof early.promise.then !== "function") return null;
        try {
          const result = await early.promise;
          if (!result || !result.ok || !result.json?.ok) return null;
          return result.json;
        } catch (e) {
          return null;
        }
      ...
      const requestRecent = async (useEarly) => {
        const earlyJSON = useEarly ? await readEarlyRecent(apiRecent) : null;
        if (earlyJSON) return earlyJSON;
...
const fetchRecent = async (useEarly = false) => {
        setLoadingRecent(true);
        setRecentErr("");
        try {
          const json = await requestRecent(useEarly);
      ...
      React.useEffect(() => { fetchRecent(true); }, [apiRecent]);
            ...
            React.createElement("div", { className: "rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm" },
              React.createElement("div", { className: "flex items-center gap-4" },
                ...
                React.createElement("button", {
                  onClick: doSearch,
                  className: "sgwb-search-btn rounded-xl bg-slate-900 text-white dark:bg-white dark:text-slate-900 px-5 py-3 text-sm font-extrabold hover:opacity-90 transition"
...
React.createElement("div", { className: (hasSearched ? "" : "hidden ") + "mt-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm" },
  React.createElement("div", { className: "px-6 py-5 border-b border-slate-200 dark:border-slate-800" },
...
React.createElement("div", { className: (hasSearched ? "hidden " : "") + "mt-10 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm" },
  ...
  try {
    if (!window.__statgroundSetMainCalled) {
      set_main();
