  ...
  function t(lang, key, fallback) {
    ...
    if (lang === "ko") {
      const ko = {
        ...
        "wb.card.shopping.title": "Shopping Deal OS",
        "wb.card.shopping.desc": "가격 검증, 딜 신뢰도, 셀러 경쟁 인텔리전스를 확인합니다."
      };
      if (ko[key]) return ko[key];
    }
    const en = {
      "wb.card.shopping.title": "Shopping Deal OS",
      "wb.card.shopping.desc": "Validate deals and inspect seller competition signals."
    };
  ...
  function render(langOverride) {
    ...
    root.innerHTML = [
      ...
      cardHTML(
        shoppingHref,
        iconShopping(),
        t(lang, "wb.card.shopping.title", "Shopping Deal OS"),
        t(lang, "wb.card.shopping.desc", "Validate deals and inspect seller competition signals."),
        cta
      ),
      '</div>',
      '</div>',
