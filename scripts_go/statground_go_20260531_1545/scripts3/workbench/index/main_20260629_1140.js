(function () {
  ...
  function pageContext() {
    if (window.STATGROUND_PAGE_CONTEXT && typeof window.STATGROUND_PAGE_CONTEXT === "object") {
  ...
  function isAdminRole(role) {
    ...
    return raw === "관리자" || normalized === "admin" || normalized === "administrator";
  }
  ...
  function t(lang, key, fallback) {
    ...
    if (lang === "ko") {
      const ko = {
        ...
        "wb.card.shopping.title": "Shopping Price Insight",
        "wb.card.shopping.desc": "가격대 분포, 할인 강도, 셀러 기회 신호를 분석합니다."
      };
      if (ko[key]) return ko[key];
    }
    const en = {
      "wb.card.shopping.title": "Shopping Price Insight",
      "wb.card.shopping.desc": "Analyze price bands, discount pressure, and seller opportunity signals."
    };
  ...
  function iconShopping() {
    return [
      '<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">',
      '<path stroke-width="2" d="M4 19h16"></path>',
      '<path stroke-width="2" d="M6 17V9"></path>',
      '<path stroke-width="2" d="M10 17V5"></path>',
      '<path stroke-width="2" d="M14 17v-7"></path>',
      '<path stroke-width="2" d="M18 17V7"></path>',
      '</svg>'
    ].join("");
  }
  ...
  function render(langOverride) {
    ...
    const showShopping = isAdminRole(pageContext().role);
    const cards = [
      cardHTML(bookHref, iconBook(), t(lang, "wb.card.book.title", "Book Workbench"), t(lang, "wb.card.book.desc", "Quickly explore book data and check quality."), cta),
      cardHTML(lectureHref, iconLecture(), t(lang, "wb.card.lecture.title", "Lecture Workbench"), t(lang, "wb.card.lecture.desc", "Explore Inflearn lecture data and collection status."), cta)
    ];
    if (showShopping) {
      cards.push(cardHTML(shoppingHref, iconShopping(), t(lang, "wb.card.shopping.title", "Shopping Price Insight"), t(lang, "wb.card.shopping.desc", "Analyze price bands, discount pressure, and seller opportunity signals."), cta));
    }
    const gridClass = showShopping ? "mt-8 grid grid-cols-1 lg:grid-cols-3 gap-4" : "mt-8 grid grid-cols-1 lg:grid-cols-2 gap-4";
