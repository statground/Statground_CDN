(function () {
  ...
  function t(lang, key, fallback) {
    ...
    if (lang === "ko") {
      const ko = {
        ...
        "wb.card.shopping.title": "Shopping Radar",
        "wb.card.shopping.desc": "쇼핑 가격대, 할인 신호, 카테고리 벤치마크를 관찰합니다."
      };
      if (ko[key]) return ko[key];
    }
    const en = {
      "wb.card.shopping.title": "Shopping Radar",
      "wb.card.shopping.desc": "Observe price ranges, discount signals, and category benchmarks."
    };
  ...
  function iconShopping() {
    return [
      ...
      '<path stroke-width="2" d="M9 15h9"></path>',
  ...
  function render(langOverride) {
    ...
    root.innerHTML = [
      ...
      cardHTML(
        shoppingHref,
        iconShopping(),
        t(lang, "wb.card.shopping.title", "Shopping Radar"),
        t(lang, "wb.card.shopping.desc", "Observe price ranges, discount signals, and category benchmarks."),
        cta
      ),
      '</div>',
      '</div>',
