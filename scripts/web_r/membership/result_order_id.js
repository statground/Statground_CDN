...
{
	$.ajax({
		url : "/membership/ajax_finish_order_id/?paymentKey=" + paymentKey + "&orderID=" + orderID,
		success:function(data){
			...
			// 가상 계좌 - 결제 대기
				...
				html += '<p style="color:blue">결제 완료시 회원 등급 만료일: ' + data.expired_at + '</p>'
				html += '<br/>'
			...
			// 결제 완료
				...
				html = '<p style="color:black; font-size:small;"><b>주문 번호: ' + data.order_id + '</b></p>'
				html += '<p style="color:black">' + data.product_name + '</p>'
				html += '<p style="color:blue">회원 등급 만료일: ' + data.expired_at + '</p>'
			}
