function check_web_r_user()
{
	...
	// 버튼 - 다음 단계로
	function btn_disabled(email)
	{
		$("#div_buttons").html("")

		html = html_button(color="success", msg="업그레이드 불가", onclick=null, disabled=true)
		html += btn_back()
	...
	$.ajax({
		...
		success:function(data){
			...
			if (data.user_name != null && data.user_name != "wrong_email") {
				...
				// 관리그룹은 업그레이드 불가
				if (data.group_name.includes("관리그룹")){
					btn_disabled()
					$("#alert_success").hide()
					$("#alert_disabled").show()

				// 유료 회원의 경우 기존 결제일, 만료일을 표시하여 준다.
				} else if (data.group_name.includes("정회원") || data.group_name.includes("VIP회원") || data.group_name.includes("기관회원")) {
					card_content += '<br/>'
					card_content += "<p style='font-size:small;'>"
					...
					card_content += "</p>"
					card_content += "<p style='font-size:small; color:red;'>" + data.group_name + " 만료일: " + data.expired_at + "</p>"
				}
