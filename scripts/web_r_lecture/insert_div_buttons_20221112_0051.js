	...
	$.ajax({
		...
		success:function(data){
			...
			if (data.success_count >= 50) {
				...
				html += '<button type="button" class="btn btn-warning" onclick="location.href=\'/application\'">'
				html += '수강 신청 하기'
				html += '</button>'
			}

			html += '<button type="button" class="btn btn-info" onclick="location.href=\'/confirm\'">'
			html += '등록 확인'
			html += '</button>'
