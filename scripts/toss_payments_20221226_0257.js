function request_order_id(product_id="", result_url="", method="", email="", username="", phone="")
{
	url = "/ajax_request_order_id/?product_id=" + product_id + "&email=" + email
		...
		success:function(data){
			...
			if (method == "card") {
				func_billing_card(clientKey=data.clientKey, orderID=data.orderID, orderName=data.product_name, amount=data.amount,
									  username=data.username, successURL=result_url, failedURL=result_url)
			} else if (method == "va") {
