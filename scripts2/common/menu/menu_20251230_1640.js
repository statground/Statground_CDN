...
// ✅ children URL 생성: href 우선, 없으면 기존 규칙(/data/{slug}/)
function childUrl(c) {
  return c?.href || `/data/${c.slug}/`;
}
              ...
              <ul class="my-2 space-y-2">
                {m.children.slice(0, Math.ceil(m.children.length/2)).map((c) => (
                  <li key={c.slug} class="flex flex-row justify-center items-center w-full">
                    {/* ✅ href 우선 적용 */}
                    <a href={childUrl(c)} class="px-4 py-2 w-full text-center hover:bg-blue-100">{c.label}</a>
                  </li>
                ))}
              </ul>
              <ul class="my-2 space-y-2">
            ...
            return (
              <React.Fragment key={m.id}>
                ...
                <div id={elemIdMobile(m.id)} class="hidden">
                  {m.children.map((c) => (
                    // ✅ href 우선 적용
                    <MobileChild key={c.slug} title={c.label} url={childUrl(c)} />
                  ))}
                </div>
              </React.Fragment>
            );
