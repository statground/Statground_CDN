...
// 출간일 기반 정렬 키(YYYYMMDD 정수). 값이 없거나 파싱 불가면 0.
function publishedSortKey(v) {
  const s = String(v ?? "").trim();
  if (!s) return 0;
  const digits = s.replace(/\D/g, "");
  if (digits.length >= 8) return parseInt(digits.slice(0, 8), 10) || 0;
  if (digits.length === 6) return parseInt(digits + "01", 10) || 0;     // YYYYMM -> YYYYMM01
  if (digits.length === 4) return parseInt(digits + "0101", 10) || 0;   // YYYY -> YYYY0101
  return 0;
}

function getPublishedValue(row) {
  // 서버 통합뷰/외부 소스 normalize 모두 커버
  return row?.published_date ?? row?.published ?? row?.pubdate ?? "";
}
	...
	function Div_main() {
	  ...
	  const fallbackRows = useMemo(() => {
	    ...
	    const rowsRaw = (Array.isArray(mergedRows) && mergedRows.length > 0)
	? mergedRows
	: (fallbackRows || []);

  // ✅ 최신 출간일(또는 연도) 기준 내림차순 정렬 (값이 없는 항목은 뒤로)
  const rows = useMemo(() => {
	const arr = Array.isArray(rowsRaw) ? rowsRaw.slice() : [];
	arr.sort((a, b) => {
	  const ka = publishedSortKey(getPublishedValue(a));
	  const kb = publishedSortKey(getPublishedValue(b));
	  if (!ka && !kb) return 0;
	  if (!ka) return 1;   // a(날짜 없음) -> 뒤로
	  if (!kb) return -1;  // b(날짜 없음) -> 뒤로
	  return kb - ka;      // 최신 먼저
	});
	return arr;
  }, [rowsRaw]);
	  ...
	  // ✅ 초기 화면: (도서 검색 + 카테고리) 전체를 '세로 중앙'에 배치
	  //    - 별도의 DOM 옵저버/리사이즈 로직 없이, flex로 안정적으로 중앙 정렬
	  if (!hasSearched) {
		return (
		  <div className="w-full flex-1 flex items-center justify-center bg-white">
			<div className="w-full max-w-6xl px-4 py-10">
			  <div className="rounded-2xl border border-slate-500 bg-white shadow-sm p-4 md:p-5 max-w-4xl mx-auto">
	...
	<button
	  ...
	  className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-5 py-2 text-sm font-semibold text-slate-800 shadow-sm hover:bg-slate-50 hover:shadow transition"
	>
	  <span>{showAllMerged ? "접기" : "더 보기"}</span>
	  <span className="text-slate-400">·</span>
	  <span className="text-slate-500">{showAllMerged ? "일부만 보기" : `총 ${countAll}건 중 더 보기`}</span>
	  <svg className={"w-4 h-4 transition " + (showAllMerged ? "rotate-180" : "")} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
		<path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7" />
			...
			{/* 초기 화면(!hasSearched)은 위에서 return으로 처리 */}
		  </div>
		</div>
	  );
	}
