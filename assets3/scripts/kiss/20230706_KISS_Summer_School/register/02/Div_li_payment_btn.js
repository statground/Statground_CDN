	...
	return (
		<div class="bg-indigo-50">
			...
			<div class="flex flex-row w-full h-full justify-center items-center text-start mx-auto">
				{
					(document.getElementById("file_input").value == '' && chk_student == 1) || (price_days_1 + price_days_2 == 0)
					? <button type="button" class={class_btn_disabled}>
						카드 결제
					  </button>
					: <button type="button" class={class_btn_enabled}
							  onClick={() => request_order_id(product_id, "1", "card", email, name, phone)}>
						카드 결제
					  </button>
				}
				{
					...
					: <button type="button" class={class_btn_enabled}
							  onClick={() => request_order_id(product_id, "1", "va", email, name, phone)}>
						무통장 입금
					  </button>
				}
				{
					...
					: <button type="button" class={class_btn_enabled}
							  onClick={() => request_order_id(product_id, "1", "account", email, name, phone)}>
						계좌 이체
					  </button>
				}
			</div>
			...
			<div>
				...
				{
					document.getElementById("file_input").value == '' && chk_student == 1
