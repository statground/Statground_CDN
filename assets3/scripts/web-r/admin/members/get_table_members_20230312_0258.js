...
async function get_table_members(type){
	let columns = null;
	let columnDefs = null;
	let chartData = null;
	...
	if (type == "daily" && toggle_table_daily == 0) {
		...
		columnDefs = data.daily.columnDefs;
		...
		columnDefs = data.monthly.columnDefs;
		...
		columnDefs = data.yearly.columnDefs;
